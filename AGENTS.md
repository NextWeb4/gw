...
- 全局查找只能从 `App` 已加载的十六个导航定义和六类业务数组派生只读结果；不得直接查询 RxDB/IndexedDB、调用 Fetch/Electron IPC、持久化索引或缓存最近搜索。导航结果调用既有 `navigate`，业务结果调用既有导航与 `selectBusinessRecord`，不得新增第二套读取、编辑或保存路径。
- 六类业务台账的关键词、结构化筛选和排序必须通过共享的类型化纯函数与统一控制条实现；筛选/排序前后不得原地修改业务数组。各模块视图状态只保留在当前 React 会话，切换模块可恢复，但不得写入 IndexedDB、localStorage、快照或同步服务。物资库存始终基于全部物资记录计算，不得随可见筛选变化。
- `apps/web/src/agenda.ts` 只负责把六类已加载数组规范化为只读日历事件、生成周一起始月网格和执行本地筛选/排序；日期来源固定为任务 `deadline`、会议 `meetingTime`、文件 `docDate`、外出 `researchTime`、用章 `sealTime`、物资 `handlerTime`。`AgendaView` 只负责展示和回调，不得读取数据库、Fetch、IPC、持久化视图状态或直接修改记录日期。
...
- `docs/HELP.md` 必须与根 `package.json` 版本一致，并覆盖工作台、事务日历、任务、会议、文件、外出、用章、物资、常用项管理、写作、周报、统计、AI 助手、历史档案、迁移、关于与设置十六个实际导航模块；修改模块名称、入口、保存语义、公开 AI、分版能力或安装资产名时必须同步说明书和文档契约测试。
...
- 六类业务台账必须支持会话级关键词、模块字段筛选、明确排序、可见数/总数和一键清除；切换模块后返回时保留当前会话视图状态，刷新页面后无需保留。
- 事务日历必须提供周一起始的 42 日月网格、前后月、回到今天、六类记录筛选和选中日期议程；空值、无效日期、六位年份不得出现。点击议程记录必须清除可能排除它的台账视图并复用原台账选中与右侧详情，日历本身不得编辑或改期。
...
- 全局查找必须验证顶栏 44×44px 入口、`Ctrl/Cmd+K`、十六个导航结果、六类业务分组、关键词筛选、上下键、Enter、Escape、焦点恢复和空状态；选择业务结果后必须进入原模块并选中原记录，桌面显示既有右侧详情，窄屏使用既有详情滚动链路，不得出现页面级横向溢出。
- 事务日历必须验证前后月、今天、六类筛选、无效日期排除、同日排序、今日/选中态、无记录日期、长标题、点击议程后的原详情链路；390×844 下日期和议程按钮至少 44px，月网格与议程纵向排列且不得横向溢出。
...
- 台账视图 Review 必须覆盖六个模块的筛选选项、默认/自定义排序、无匹配与清除状态、切换模块后的会话恢复、源数组未被排序修改，以及物资筛选后库存仍按全部记录计算；390px 下筛选控件须换行且按钮/select 至少 44px 高。
- 事务日历 Review 必查固定日期映射、四位年份与真实日期校验、周一起始跨月网格、类型筛选、同日多记录、键盘按钮、移动端无溢出和既有详情复用；禁止拖拽改期、日期写回、视图持久化、网络请求或第二套保存逻辑。
