...
- 全局查找只能从 `App` 已加载的十七个导航定义和六类 active 业务数组派生只读结果；trash 与 purged 记录不得进入业务搜索。不得直接查询 RxDB/IndexedDB、调用 Fetch/Electron IPC、持久化索引或缓存最近搜索。导航结果调用既有 `navigate`，业务结果调用既有导航与 `selectBusinessRecord`，不得新增第二套读取、编辑或保存路径。
...
- 全局查找必须验证顶栏 44×44px 入口、`Ctrl/Cmd+K`、十七个导航结果、六类 active 业务分组、关键词筛选、上下键、Enter、Escape、焦点恢复和空状态；trash/purged 不得出现在记录结果中。选择业务结果后必须进入原模块并选中原记录，桌面显示既有右侧详情，窄屏使用既有详情滚动链路，不得出现页面级横向溢出。
