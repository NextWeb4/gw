...
- `pnpm test:e2e` 使用 Playwright 验证离线页面、任务协同字段、附件暂存/保存/解除关联/引用清理、删除确认、历史导入、文稿/周报版本历史、文稿导出和周报保存/导出。CI 必须先执行 `pnpm exec playwright install --with-deps chromium`。
...
- 全局命令面板只能从 `App` 已加载的十七个导航定义、六类 active 业务数组和六个静态快速新建动作派生结果；trash 与 purged 记录不得进入业务搜索。不得直接查询 RxDB/IndexedDB、调用 Fetch/Electron IPC、持久化索引、命令状态或最近搜索。导航结果调用既有 `navigate`，业务结果调用既有导航与 `selectBusinessRecord`；快速新建只能关闭面板后调用对应 `empty*` 和 `open*Editor`，不得自动保存或新增第二套读取、编辑、附件与保存路径。
...
- 工作台时间概览必须从 `apps/web/src/agenda.ts` 的规范化事件与同一批已加载数组派生，不得复制六类日期映射。逾期只允许未完成任务进入；过去的其他业务记录直接排除；无日期或无效日期进入未排期；已完成任务全部排除。概览组件只能维护当前会话切换状态并回调既有记录打开入口，不得直接读写数据适配层。
- 快速记录组件只负责会话文本、确定性识别预览和“继续核对”回调；字段应用必须复用 `packages/domain` 的纯函数，最终编辑必须打开现有 `TaskEditor`。快速记录不得导入 `packages/local-data`、Fetch 或 Electron IPC，也不得直接调用保存函数；从快速记录进入的任务抽屉必须保留原文并从打开时即视为未保存草稿。
...
- 六类业务生命周期由 `packages/domain` 纯函数集中处理：active 无删除标记，trash 具有 `deletedAt` 且保留完整 payload，purged 只能是 `id/updatedAt/deletedAt/purgedAt` 四字段墓碑。`App` 只把 active 交给台账/工作台/日历/统计/周报/AI/目录/搜索，把 trash 交给 `RecycleBinView`，把三种状态全量交给显式私有同步。
- 六类 active 业务的“复制相似记录”必须调用 `packages/domain` 的统一纯函数生成新 ID、新时间戳和独立嵌套数组，并移除删除标记、迁移版本与 `legacyPayload`。任务进度/配合状态/阶段状态/工作小结、文件登记状态和外出成果按领域规则重置；主附件只共享不可变本机附件 ID。复制入口只允许打开原编辑抽屉并从打开时标记为未保存，禁止自动写库、复制附件二进制、复制回收站记录或形成第二套保存逻辑。
- 新选附件必须先停留在编辑会话，保存业务记录时再持久化；取消编辑不得产生孤立附件。解除关联或永久删除记录时，只能清理已不被其他 active/trash 业务记录或历史档案引用的候选附件；软删除必须保留附件。内网同步只上传按 `updatedAt` 决出的有效 master 实际引用的附件，远端 purge 覆盖本机旧 payload 后必须清理孤立附件。
- `docs/HELP.md` 必须与根 `package.json` 版本一致，并覆盖工作台、事务日历、任务、会议、文件、外出、用章、物资、常用项管理、写作、周报、统计、AI 助手、回收站、历史档案、迁移、关于与设置十七个实际导航模块；修改模块名称、入口、保存语义、公开 AI、分版能力或安装资产名时必须同步说明书和文档契约测试。
...
- 主草稿与每份已保存周报的版本历史共用 `DocumentRevision` 和本机 `document-revision` setting：只在原“保存版本”成功后记录，恢复只载入原编辑器并保留当前 ID/版本，再次通过原按钮保存。单条完整序列化快照最多 1,000,000 字符，每目标 20 条、全局 100 条且总量 10,000,000 字符；历史进入本地快照但不得进入私有同步、全局查找、AI 材料、日志或网络。外层/负载 ID、target/snapshot ID、版本和时间必须一致，删除按 setting kind，快照导入必须先预检全部跨 kind ID 冲突再写入。
...
## 8. 完成标准
- 任务、会议、文件、外出、用章、物资、写作、周报、回收站、历史档案和关于页均可本地使用；六类业务移入回收站前必须确认，恢复、永久删除和清空回收站也必须明确确认；周报只汇总 active 业务模块并覆盖人工编辑、版本保存、快照恢复和 DOCX/PDF 导出；历史 Skill、配置及 `legacyPayload` 必须以纯文本只读显示。
...
- 工作台必须提供“今日与逾期 / 未来 7 天 / 未排期”跨模块概览，并保持工作台首屏以可操作记录为中心；三类分桶、数量、排序和记录跳转必须由纯函数单元测试与桌面/移动 E2E 锁定，不得形成第二套编辑、完成或改期入口。
- 顶栏和工作台必须共用同一个“快速记录任务”入口；`Shift+A` 只在非编辑目标、无其他模态层时打开。识别结果必须显示字段预览并在原任务抽屉中继续核对，保存仍走 `saveTask`；取消快速记录不得创建记录，关闭预填抽屉必须经过未保存确认。
- 六类业务详情必须提供“复制相似记录”，复制草稿明确说明未保存、字段重置和附件共享语义；取消或关闭必须立即走统一未保存守卫且不得新增记录，保存必须通过原必填/日期/数量校验并在清除当前台账筛选后选中新记录。
- 任务状态以中文显示；交办人、承办人和单位可跨记录复用；工作小结可从三类确定性模板生成并继续编辑。
- 写作中心可导入 DOCX/HTML/TXT，保存本机自定义格式并在刷新后复用；导入清洗和 10 MB 上限必须有回归测试。
- 本机自定义写作格式可重命名和确认删除，授权知识包格式必须只读且不得显示管理入口。重命名必须复用 `custom-template:${template.id}` setting，只更新规范化名称和 `updatedAt`；快照解析必须拒绝外层 setting ID 与 payload 模板 ID 不一致的自定义格式，删除必须核对实际记录 kind。删除当前关联格式只能清空当前 React 草稿的弱模板引用，不得改变标题、正文、文种、版本或已保存文稿；格式管理成功后只能局部更新模板状态，不得用全局重载覆盖未保存草稿。管理动作不得新增 schema、同步集合、API、IPC、后台任务或网络请求。
- 主草稿和周报必须能通过共用原生对话框按目标查看只读当前/历史文本、确认后恢复为持续标记的未保存工作副本、删除单条旧版本和清空当前目标历史；当前保存版不得单独删除。保存和历史操作必须有忙碌态并局部更新，周报删除取消不得清空编辑态；对话框打开时全局查找、快速记录和 AI overlay 互斥，390×844 下操作至少 44px且不得遮挡固定底栏。
...
- 自定义写作格式管理必须验证：空白/超长名称拒绝、稳定 setting ID、正文与结构不变、Enter 保存、Escape/取消及焦点恢复、删除确认取消/确认、当前文稿不变、快照删除、授权格式无管理入口、管理动作零请求，以及 390×844 下操作按钮至少 44×44px 且无横向溢出。
...
- 全局命令面板必须验证顶栏 44×44px 入口、`Ctrl/Cmd+K`、十七个导航结果、六类 active 业务分组、六类快速新建、动作同义词筛选、上下键、Enter、Escape、焦点时序和空状态；trash/purged 不得出现在记录结果中。选择业务结果后必须进入原模块并选中原记录；选择快速新建后必须先关闭面板，再打开原编辑抽屉并聚焦首字段，取消零写入，桌面与窄屏均不得横向溢出。
- 回收站必须验证六类记录搜索、类型筛选、恢复、永久删除、清空、软删除期间附件下载、恢复后附件继续可用、共享附件不误删、永久删除快照不含业务正文、tombstone 显式同步和远端 purge 后附件清理；390×844 下搜索、筛选和操作按钮至少 44px 且无页面级横向溢出。
- 复制相似记录必须用领域单测覆盖六类新 ID/时间戳、源对象不变、生命周期与迁移字段剥离、任务/文件/外出状态重置和主附件引用共享；E2E 必须覆盖六类详情入口、立即未保存、取消零写入、任务保存后原记录仍存在、新记录被选中、零外联，以及 390×844 下操作按钮至少 44px。
...
- 工作台概览 Review 必查：未完成任务逾期优先、当天六类事项、未来 7 天边界、无效日期与空日期进入未排期、已完成任务和过去非任务排除、稳定排序、空状态、长标题、既有详情复用；1440×900 首屏应露出主要行动区，390×844 下切换与记录按钮至少 44px 且无页面级横向溢出。
- 快速记录 Review 必查：顶栏与工作台入口一致、快捷键不劫持表单输入、识别预览只使用确定性规则、无命中也可继续手工核对、原文在任务抽屉可见、字段应用不重复实现、预填抽屉立即受未保存保护、取消/保存后原文清空、与全局查找/AI/六类抽屉不叠层；390px 下按钮至少 44px 且原生 dialog 不被固定底栏遮挡。
...
- 全局命令面板 Review 必查 cmdk 只承载可访问组合框/列表语义，结果模型仍由项目明确提供；检查稳定 action ID/value/keywords、六类动作完整性、敏感字段排除、活动项和焦点环、普通关闭焦点恢复、动作选择后首字段焦点、长中文文本、无结果状态、取消零写入，以及编辑抽屉/快速记录/AI overlay 打开时不会叠加模态层。
- 回收站 Review 必查领域纯函数不修改源对象、恢复保留完整业务字段和附件、墓碑严格只有四字段、快照对墓碑再次最小化、软删除零附件清理、永久删除只清理无引用附件、远端较新墓碑胜过本机旧 payload，并证明台账/工作台/日历/统计/周报/AI/目录/搜索都只消费 active。
- 复制相似记录 Review 必查逐字段白名单而非对象盲拷贝、新 ID/时间戳、阶段新 ID、结果状态重置、迁移证据不带入、主附件 ID 数组独立且二进制不复制、原记录不变、复制草稿立即受未保存守卫、取消零写入、保存复用原校验并清除可能隐藏新记录的会话筛选。
- 文稿版本历史 Review 必查显式保存才写历史、两目标共用白名单快照和裁剪、恢复保留 canonical ID/版本且刷新回到 head、再次原保存生成更高版本、周报按 report ID 隔离、取消删除保持编辑态、单条/每目标/全局/总量上限、kind 安全读删、导入冲突零部分写入、同步/搜索/AI/网络排除，以及桌面/390×844 对比、滚动、46px 和回焦。
...
- 复制草稿若直接展开原对象会携带 `deletedAt`、`purgedAt`、`sourceVersion`、`legacyPayload`、旧阶段 ID 或已完成状态；若深复制附件二进制会扩大本机存储并破坏共享引用清理。必须使用领域白名单转换和原附件 ID 引用模型。
...
- 回收站只扩展 payload 的可选生命周期字段，RxDB 外层 schema 与 kind 未变化时不得为“看起来安全”无故提升版本；但移除墓碑解析会把已永久删除记录错误当成业务数据，未来回滚 UI 时也必须保留 tombstone 兼容。
