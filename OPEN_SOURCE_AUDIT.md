...
问题本质：写作中心已经能把当前标题与正文另存为本机自定义格式，并在刷新和快照恢复后复用，但每次保存只能新增，名称录错后无法修正，失效格式也无法清理。授权知识包格式与本机格式显示在同一列表中，因此管理能力必须根据领域类型隔离，不能把“同一列表”误解为“相同所有权”。格式被套用后，当前文稿只是复制其内容并保留一个弱 `templateId` 引用；管理格式不得反向改写文稿。

第一性原理不变量：只有 `CustomWritingTemplate` 可以管理；重命名只能规范化名称并更新时间，稳定模板 ID、正文、结构、创建时间和来源不得改变；删除必须明确确认且只移除原 setting 记录；删除当前关联格式最多清空内存草稿的弱引用，不得清空标题、正文、文种或版本。快照中的外层 setting ID 必须精确等于 `custom-template:${payload.id}`，删除还必须核对实际记录类型。所有操作必须复用 `packages/local-data` 的既有 setting 生命周期，不新增数据库 schema、同步集合、私有 API、Electron IPC、后台任务或网络请求。

| 方案名称 | 来源 / 许可证 | 可借鉴逻辑 | 冲突与限制 | 是否采用 / 采用方式 |
| --- | --- | --- | --- | --- |
| LibreOffice Template Manager | [官方帮助](https://help.libreoffice.org/latest/en-US/text/shared/guide/template_manager.html)、[LibreOffice 许可证](https://www.libreoffice.org/about-us/licenses) / MPL-2.0 等 | 用户模板可重命名、取消保留原名并在确认后删除；内置模板不可管理 | 完整分类、菜单、批量操作、桌面 UI 与源码均超出本项目边界 | 只借鉴“内置只读、用户模板可改名并确认删除”；不复制源码、文案、视觉或分类模型 |
| BookStack Page Templates | [官方文档](https://www.bookstackapp.com/docs/user/page-templates/)、[许可证](https://github.com/BookStackApp/BookStack/blob/development/LICENSE) / MIT | 套用模板时复制内容，后续修改模板不会反向改变已经使用的页面 | 服务端页面、角色权限、拖拽插入和默认模板与本机单草稿模型冲突 | 只借鉴“套用后文稿独立”；删除格式不改当前或既有文稿 |
| Joplin Templates plugin | [插件页](https://joplinapp.org/plugins/plugin/joplin.plugin.templates/)、[许可证](https://github.com/joplin/plugin-templates/blob/master/LICENSE) / MIT | 模板复用普通笔记的存储与生命周期，不建立第二套模板数据库 | Handlebars、变量、默认模板、插件协议和同步能力不属于本次需求 | 只借鉴复用既有生命周期；继续使用原 setting 记录和本地数据适配器 |
| 稳定 setting ID + 类型受限的行内管理 | 当前项目 / UNLICENSED | `custom-template:${template.id}` 已是保存、读取、快照和删除的统一身份 | 必须防止误管授权格式、筛选后失焦、空名称、超长名称和删除当前引用时内容丢失 | 采用；领域纯函数重命名，React 只编排适配器调用、确认、筛选和焦点 |
| 新建模板管理表、同步集合或私有 API | 自研 / 不适用 | 可扩展批量管理、云端共享或权限控制 | 扩大 schema、同步、鉴权、冲突、迁移和秘密边界，且不能解决当前本机缺口 | 不采用 |

- 所有权边界：`'custom' in template` 是管理入口的领域门；官方规范、单位模板、授权教材建议和其他知识包格式不显示铅笔或删除按钮。
- 重命名边界：名称去除首尾空白，不能为空且上限 80 个字符；只更新 `name` 和 `updatedAt`，并以原 setting ID 覆盖保存。取消不写入，Enter 与显式保存按钮走同一提交函数。
- 删除边界：原生确认取消时零写入；确认后用 `removeRecordOfKind('setting', ...)` 只删除 `custom-template:${template.id}`。若当前草稿引用该格式，只清空 `templateId`；标题、正文、文种、版本和其他草稿字段保持不变。异常快照身份在 `parseLocalSnapshot` 入口跳过，既有错配数据也不能借删除动作移除其他 kind。
- 持久化与联网：继续调用注入到 App 的 `putRecord/removeRecordOfKind`，React 组件不直接访问 IndexedDB。新增、重命名和删除成功后只局部更新 `customTemplates`，不得调用会重载 `draft_main` 的全局 `reload()`；三项操作均不触发 Fetch、桌面 IPC、同步、快照写出或外部服务。
- 可访问性：图标按钮保留无障碍名称与 tooltip；行内输入自动聚焦，Escape 取消后焦点返回原重命名按钮，删除后焦点返回搜索框；390px 下操作按钮至少 44×44px。
- 审查修复：真实浏览器复现了全局刷新覆盖未保存文稿、图标聚焦时 Escape 失效和错配快照身份可能跨类型删除；分别通过模板状态局部更新、表单级键盘守卫、规范身份解析与类型安全删除修复，并加入回归。
- 回滚：移除领域重命名纯函数、两项行内动作和相应 UI 测试即可恢复 v0.7.11；快照身份与类型安全删除保护可以保留。现有 setting payload、快照格式、数据库 schema、同步协议和服务端无需迁移或回滚。
...
问题本质：六类业务台账已经具有统一的只读详情、编辑抽屉、附件会话、必填与日期校验、未保存守卫和本机持久化，但用户重复登记同类任务、会议、文件、外出、用章或物资时，只能从空表单重新输入。所缺能力不是数据库复制、批量克隆、模板系统或后台任务，而是把一条 active 记录确定性转换为“尚未保存的新草稿”，交回原编辑器人工复核。

第一性原理不变量：输入只能是当前已加载的 active 业务记录；复制必须生成新 ID、`createdAt` 和 `updatedAt`，移除 `deletedAt`、`purgedAt`、`sourceVersion` 和 `legacyPayload`，且不得修改原对象或复用可变嵌套数组；主附件只共享现有不可变本机附件 ID，不复制二进制、不上传、不自动写库，复制草稿解除关联也不能影响原记录；任务保留标题、分类、来源、交办人与业务日期，但状态重置为未启动、工作小结清空、配合单位和阶段状态重置、阶段生成新 ID、阶段/单位进度附件清空；文件登记状态重置为“待登记”；外出活动旧成果清空；其余业务字段、日期、备注和主附件引用保留供人工调整。详情栏点击复制后只能打开既有抽屉，并从首屏明确显示“未保存的新记录”；取消、关闭、外侧、Escape 和浏览器离开必须立即受到原未保存守卫保护；只有用户点击原保存按钮并通过原校验后才允许写入 IndexedDB，新记录保存后选中其既有详情。整个过程不得新增 Fetch、IPC、服务端路由、同步协议、数据库 schema、自动联网、持久化复制队列或第二套保存逻辑。

| 方案名称 | 来源 | 许可证 | 核心能力 | 优点 | 缺点 | 维护状态 | 与当前项目的契合度 | 可能冲突点 | 是否采用 | 采用方式 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 现有 TypeScript 领域纯函数 + 六类既有编辑抽屉 | 当前项目、React 19、Lucide | 项目 UNLICENSED / React MIT / Lucide ISC | 类型化逐字段复制、业务状态重置、附件 ID 共享、强制未保存、原校验与保存复用 | 零新增依赖；能明确排除生命周期和迁移证据；与附件引用清理、三栏详情、移动端和本机数据边界完全一致 | 需要项目维护六类字段映射和对应回归测试 | 当前项目持续维护 | 高 | 若直接展开对象会把墓碑/迁移字段带入；若调用保存函数会破坏人工复核不变量 | 采用 | `packages/domain` 提供一个重载纯函数，`App` 只负责打开原抽屉、显示复制提示和在保存后选中新记录 |
| Vikunja TaskDuplicate `506dbd7b` | [官方源码](https://github.com/go-vikunja/vikunja/blob/506dbd7b0483c626295081fd9dd5beef3df8def7/pkg/models/task_duplicate.go) | AGPL-3.0-or-later | 逐字段复制任务、重置完成状态、复制标签/指派/提醒/附件并建立 copied-from 关系 | 明确区分新身份和完成状态；证明复制语义应逐字段定义而不是盲目克隆 | 服务端立即创建；复制附件二进制和关系；单一任务模型；AGPL 不可复制进当前保留全部权利客户端 | 审计时仓库 HEAD 为 `506dbd7b` | 只作领域语义参考 | 直接复用会引入 AGPL、服务端、权限、附件复制和自动保存冲突 | 只借鉴设计 | 仅借鉴“新身份、完成状态重置、逐字段白名单”，不复制源码、接口、文案、关系或附件实现 |
| Web `structuredClone` | [MDN](https://developer.mozilla.org/en-US/docs/Web/API/Window/structuredClone) / Web 标准 | 结构化深复制 | 零依赖；可避免嵌套数组引用共享 | 会完整保留 `deletedAt`、`purgedAt`、迁移证据、旧阶段 ID 和业务结果状态；不能表达六类重置规则 | 目标 Chromium 原生支持 | 中低 | 盲目深复制会违反生命周期、证据边界和状态重置不变量 | 不单独采用 | 仅确认“复制不能共享可变数组”的原则；最终使用显式字段映射 |
| lodash.clonedeep 4.5.0 | [官方仓库](https://github.com/lodash/lodash) / [npm](https://www.npmjs.com/package/lodash.clonedeep) | MIT | 通用深复制 | API 成熟，兼容旧运行时 | 2022-06-19 后 npm 元数据未更新；新增依赖仍不能决定字段保留/重置语义 | 稳定但低频维护 | 低 | 会把领域错误隐藏在通用克隆后，仍需第二次删除和重写字段 | 不采用 | 不新增依赖，显式映射更短、更可审计 |
| rfdc 1.4.1 | [官方仓库](https://github.com/davidmarkclements/rfdc) / [npm](https://www.npmjs.com/package/rfdc) | MIT | 约 27 KB 解包的快速深复制 | 体积小、实现专注、2024-06-12 有 npm 更新 | 只解决对象复制，不解决新 ID、状态重置、附件和迁移证据边界 | 维护中 | 低 | 会增加依赖却仍需完整领域适配层 | 不采用 | 不引入运行依赖；保留为未来通用大对象复制的候选，不用于业务记录 |

- 直接复用：现有 `createId`、React 受控状态、六类编辑抽屉、附件 ID 模型、未保存守卫、保存校验、详情选择和 Lucide `CopyPlus`。
- 只借鉴设计：Vikunja 的新身份、完成状态重置和逐字段白名单；Web/深复制库仅用于验证“复制后不得共享可变引用”。未复制任何外部源码、接口、文案、样式、资产或数据。
- 不采用：服务端复制端点、自动保存、复制附件二进制、copied-from 持久关系、批量克隆、通用深复制依赖、复制历史墓碑/迁移原文、自动改日期、自动联网和复制队列。
- 冲突审计：最终方案与 React/Vite、现有目录、运行/构建方式、RxDB 外层 schema、快照、私有同步、权限、离线/联网边界和保留全部权利许可证均无冲突。若后续要求自动保存、把原附件复制成新二进制、复制回收站记录、保留旧进度结果或引入 AGPL 实现，则必须停止并重新审计数据与许可证影响。
- 许可证风险：无新增依赖。Vikunja 仅作抽象设计参考；MIT 库未引入、未复制实现。
- 联网变化：无。查看详情、生成复制草稿、修改、取消和保存都不新增网络请求；私有同步仍只由用户显式触发。
- 回滚方式：移除领域复制函数、详情栏复制按钮、复制提示/状态和对应测试即可恢复 v0.7.5；无数据库、快照、同步、服务端或附件迁移。
