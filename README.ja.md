...
![バージョン](https://img.shields.io/badge/version-0.7.16-0969da?style=flat-square)
...
現在のリリースは `0.7.16` です。6種類の共通業務詳細に端末内スター切替を追加し、active なスター項目をダッシュボードと、空検索時のコマンドパレットでセッション内の最近項目より先に表示します。すべての入口は既存のレコード表示経路を再利用します。スターは最大 12 参照で、種類、ID、登録時刻だけを IndexedDB とローカルスナップショットに保存し、業務 payload、プライベート同期、AI 資料、IPC、通信には含めません。最近のリリースでは、週報の参照元への読み取り専用リンク、デスクトップ詳細の折りたたみレール、狭幅画面でのネイティブダイアログ入場境界も追加・修正しました。詳細は [`RELEASE_NOTES.md`](RELEASE_NOTES.md) を参照してください。
...
| 分野 | 実装内容 |
| --- | --- |
| 業務管理 | タスク、会議、文書、外出、押印、物資の編集可能な6台帳、グローバルなクイックタスク入力、端末内スター、セッション内の最近項目、現在の可視順を移動する前/次の詳細操作、デスクトップ詳細の折りたたみレール、現在結果の安全な CSV 出力、再利用可能な担当者/組織、段階、要約、添付、台帳内キーワード/項目フィルター/並べ替え、ローカル全体検索、統合業務カレンダー、復元可能なごみ箱 |
...
リポジトリのバージョンは `0.7.16` です。依存関係は `pnpm-lock.yaml` で固定され、再現可能なインストールには frozen lockfile を使用します。
