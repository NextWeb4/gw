...
1. タスクとファイル記録を作成または取り込み、移行レポートと読み取り専用履歴を確認します。
...
- モデルキー、データベース資格情報、provider 設定はプライベートサーバーだけに置き、クライアントストレージには保存しません。
...
公開 Pages は厳格な CSP を使い、プライベート接続先を持ちません。イントラネット/デスクトップ CSP は明示的なプライベート操作に必要な HTTPS とローカル API のみを許可します。HTML の CSP meta では `frame-ancestors` を強制できないため、埋め込み防止が必要な本番ホスティングでは HTTP 応答ヘッダーに `Content-Security-Policy: frame-ancestors 'none'` を設定してください。
