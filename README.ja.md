...
![バージョン](https://img.shields.io/badge/version-0.7.20-0969da?style=flat-square)
...
現在のリリースは `0.7.20` です。6 台帳の active レコードを読み取り専用のネイティブダイアログで横並び比較できるようになりました。比較対象は現在のセッションだけに存在し、固定フィールドの許可リスト、ローカル検索、差分のみ表示に対応します。IndexedDB、スナップショット、プライベート同期、AI 資料、IPC、URL、通信には含めません。詳細は [`RELEASE_NOTES.md`](RELEASE_NOTES.md) を参照してください。
...
リポジトリのバージョンは `0.7.20` です。依存関係は `pnpm-lock.yaml` で固定され、再現可能なインストールには frozen lockfile を使用します。
