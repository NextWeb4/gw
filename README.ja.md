...
![バージョン](https://img.shields.io/badge/version-0.7.8-0969da?style=flat-square)
...
現在のリリースは `0.7.8` です。空の `Ctrl/Cmd+K` パレットには、明示的に開いた active 業務レコードが最大6件、直近順で表示されます。履歴は React セッション内の「モジュール + レコード ID」だけで、再訪時は先頭へ移動し、検索入力中は非表示、削除時は除外、再読み込み時は消去されます。最近項目も既存の台帳、選択状態、読み取り専用詳細をそのまま使用します。詳細は [`RELEASE_NOTES.md`](RELEASE_NOTES.md) を参照してください。
...
| 分野 | 実装内容 |
| --- | --- |
| 業務管理 | タスク、会議、文書、外出、押印、物資の編集可能な6台帳、グローバルなクイックタスク入力、再利用可能な担当者/組織、段階、要約、添付、台帳内キーワード/項目フィルター/並べ替え、ローカル全体検索、統合業務カレンダー、復元可能なごみ箱 |
...
| ローカルデータ | IndexedDB ベースのリポジトリ、スナップショット、添付参照、検索可能な AI 履歴、6台帳のソフト削除/復元、最小同期トゥームストーン、明示的な書き出し |
...
リポジトリのバージョンは `0.7.8` です。依存関係は `pnpm-lock.yaml` で固定され、再現可能なインストールには frozen lockfile を使用します。
