...
![バージョン](https://img.shields.io/badge/version-0.7.19-0969da?style=flat-square)
...
現在のリリースは `0.7.19` です。上限付き・セッション限定の業務レコード履歴に、キーボードとタッチで任意の保持項目へ移動できる一覧を追加しました。移動はカーソルだけを変更し、既存のレコード表示経路を再利用します。IndexedDB、スナップショット、プライベート同期、AI 資料、IPC、URL、通信には含めません。詳細は [`RELEASE_NOTES.md`](RELEASE_NOTES.md) を参照してください。
...
| 分野 | 実装内容 |
| --- | --- |
| 業務管理 | タスク、会議、文書、外出、押印、物資の編集可能な6台帳、グローバルなクイックタスク入力、端末内スター、セッション内の最近項目とモジュール間の戻る/進む履歴、現在の可視順を移動する前/次の詳細操作、デスクトップ詳細の折りたたみレール、現在結果の安全な CSV 出力、再利用可能な担当者/組織、段階、要約、添付、台帳内キーワード/項目フィルター/並べ替え、ローカル全体検索、統合業務カレンダー、復元可能なごみ箱 |
...
リポジトリのバージョンは `0.7.19` です。依存関係は `pnpm-lock.yaml` で固定され、再現可能なインストールには frozen lockfile を使用します。
