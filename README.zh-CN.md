...
![版本](https://img.shields.io/badge/version-0.7.25-0969da?style=flat-square)
...
当前发布版本为 `0.7.25`：任务页新增会话级“列表 / 看板 / 时间轴”三种视图。时间轴从同一份筛选和排序后的任务投影派生“已过截止日期 / 今天 / 未来 7 天逐日 / 更远日期 / 未排期”分组，卡片复用原右侧只读详情；视图不写入 IndexedDB、快照或同步，不支持拖拽、内联改状态或批量写入。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
| 领域 | 已实现内容 |
| --- | --- |
| 事务管理 | 任务、会议、文件、外出、用章、物资六类可编辑台账，任务内嵌检查清单，并支持同模块 active 记录只读对比；独立维护常用人员/单位目录，以及全局快速任务记录、本机星标、会话级最近记录与跨模块后退/前进、当前可见顺序的上一条/下一条详情浏览、桌面详情折叠轨道、当前筛选结果安全 CSV 导出、阶段、小结、附件、模块内关键词/字段筛选/排序、本机全局查找、统一事务日历和可恢复回收站 |
...
仓库版本为 `0.7.25`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
