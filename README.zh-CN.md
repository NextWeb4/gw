...
![版本](https://img.shields.io/badge/version-0.7.17-0969da?style=flat-square)
...
当前发布版本为 `0.7.17`：六类业务的显式访问会在共用详情栏形成有界、仅当前会话有效的后退/前进历史。任务—文件关联、周报来源、日历、工作台、星标和全局查找的跨模块跳转都能沿原记录打开路径返回；后退后打开不同记录会截断前进分支，已删除目标会被裁剪，刷新页面会清空历史。历史只在 React 内存保存记录类型和 ID，不进入 IndexedDB、快照、私有同步、AI 材料、IPC、URL 或网络请求。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
| 领域 | 已实现内容 |
| --- | --- |
| 事务管理 | 任务、会议、文件、外出、用章、物资六类可编辑台账，独立维护常用人员/单位目录，并支持全局快速任务记录、本机星标、会话级最近记录与跨模块后退/前进、当前可见顺序的上一条/下一条详情浏览、桌面详情折叠轨道、当前筛选结果安全 CSV 导出、阶段、小结、附件、模块内关键词/字段筛选/排序、本机全局查找、统一事务日历和可恢复回收站 |
...
仓库版本为 `0.7.17`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
