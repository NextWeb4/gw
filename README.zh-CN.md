...
![版本](https://img.shields.io/badge/version-0.7.20-0969da?style=flat-square)
...
当前发布版本为 `0.7.20`：六类台账的 active 记录现在可以在只读原生对话框中并排核对。对比目标只存在当前会话，使用固定字段白名单，支持本地搜索和“仅看差异”，不进入 IndexedDB、快照、私有同步、AI 材料、IPC、URL 或网络请求。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
仓库版本为 `0.7.20`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
