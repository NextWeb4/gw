...
![版本](https://img.shields.io/badge/version-0.7.1-0969da?style=flat-square)
...
当前发布版本为 `0.7.1`：任务、会议、文件、外出、用章和物资六类台账现在共用一套本机筛选与排序控制。每个模块可组合关键词、结构化字段筛选和日期/标题排序，显示“可见数 / 总数”并支持一键清除；切换模块后会在当前会话恢复视图，刷新页面后不保留。全部转换只作用于 React 已加载数组，不联网、不写入 IndexedDB 或快照，物资库存仍按全部收发记录计算。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
| 领域 | 已实现内容 |
| --- | --- |
| 事务管理 | 任务、会议、文件、外出、用章、物资六类可编辑台账，独立维护常用人员/单位目录，并支持阶段、小结、附件、模块内关键词/字段筛选/排序和本机全局查找 |
...
仓库版本为 `0.7.1`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
