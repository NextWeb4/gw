...
![版本](https://img.shields.io/badge/version-0.7.16-0969da?style=flat-square)
...
当前发布版本为 `0.7.16`：六类共用业务详情新增本机星标开关，active 星标会显示在工作台，并在空查询命令面板中排在会话级最近访问之前；所有入口仍复用既有记录打开路径。星标最多保存 12 个引用，只把记录类型、ID 和星标时间写入 IndexedDB 与本地快照，不进入业务 payload、私有同步、AI 材料、IPC 或网络请求。近期版本还新增周报来源记录只读跳转、桌面详情折叠轨道，并修复窄屏原生对话框入场越界。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
| 领域 | 已实现内容 |
| --- | --- |
| 事务管理 | 任务、会议、文件、外出、用章、物资六类可编辑台账，独立维护常用人员/单位目录，并支持全局快速任务记录、本机星标、会话级最近记录、当前可见顺序的上一条/下一条详情浏览、桌面详情折叠轨道、当前筛选结果安全 CSV 导出、阶段、小结、附件、模块内关键词/字段筛选/排序、本机全局查找、统一事务日历和可恢复回收站 |
| 写作 | 富文本起草、经清洗的 DOCX/HTML/TXT 导入、本机自定义格式、带只读来源记录跳转的确定性周报、有界本机版本历史及其只读对比和非破坏恢复、历史档案 |
| 文档 | DOCX 与 PDF 共用的 A4 导出引擎；Web 使用浏览器打印，桌面端使用 Electron 打印 |
| 迁移 | 兼容两份历史原型导出结构和拖拽 JSON/快照；当共同版本标识无法可靠辨别来源时给出警告 |
| 本地数据 | 基于 IndexedDB 的仓储、包含文稿历史和仅引用星标的快照、附件引用、AI 历史查询、六类业务软删除/恢复、最小同步墓碑以及显式导出操作 |
...
仓库版本为 `0.7.16`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
