...
![版本](https://img.shields.io/badge/version-0.7.10-0969da?style=flat-square)
...
当前发布版本为 `0.7.10`：任务、会议、文件、外出、用章和物资六类台账均可把当前关键词、结构化筛选和排序后的可见结果直接导出为 CSV。导出使用固定业务字段白名单、UTF-8 BOM 与 CRLF，附件只输出数量，危险公式前缀会被中和；操作不读取第二份数据库、不改写记录、不持久化视图状态，也不联网。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
仓库版本为 `0.7.10`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
