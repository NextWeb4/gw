...
![版本](https://img.shields.io/badge/version-0.6.1-0969da?style=flat-square)
...
当前发布版本为 `0.6.1`：公开 Pages 和互联网版提供可选的本机中转模式，用户显式输入本机密码后可刷新后端维护的“神秘站点”、获取模型并发送已确认脱敏材料。本补丁明确声明回环目标地址空间，并在 Chrome 拒绝“本地网络访问”时给出可操作的恢复路径。上游地址、API Key、密码和配置密钥均不进入公开构建。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
