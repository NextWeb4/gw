...
![版本](https://img.shields.io/badge/version-0.7.19-0969da?style=flat-square)
...
当前发布版本为 `0.7.19`：在有界、仅当前会话有效的跨模块后退/前进历史上，新增访问轨迹列表，可用键盘或触控直接跳到任意历史项；跳转只移动游标并沿用原记录打开路径，不进入 IndexedDB、快照、私有同步、AI 材料、IPC、URL 或网络请求。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
仓库版本为 `0.7.19`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
