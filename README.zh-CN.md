...
![版本](https://img.shields.io/badge/version-0.7.21-0969da?style=flat-square)
...
当前发布版本为 `0.7.21`：六类台账的 active 记录详情现在可以一键复制固定白名单纯文本摘要。复制会等待系统剪贴板确认，只输出可见业务字段、active 关系标题和附件数量；不会写入 IndexedDB、快照、私有同步，也不会包含内部 ID、迁移原文、附件正文、密钥或 AI 材料。详情见 [`RELEASE_NOTES.md`](RELEASE_NOTES.md)。
...
| 领域 | 已实现内容 |
| --- | --- |
| 事务管理 | 任务、会议、文件、外出、用章、物资六类可编辑台账，支持同模块 active 记录只读对比；独立维护常用人员/单位目录，并支持全局快速任务记录、本机星标、会话级最近记录与跨模块后退/前进、当前可见顺序的上一条/下一条详情浏览、桌面详情折叠轨道、当前筛选结果安全 CSV 导出、阶段、小结、附件、模块内关键词/字段筛选/排序、本机全局查找、统一事务日历和可恢复回收站 |
...
仓库版本为 `0.7.21`。依赖由 `pnpm-lock.yaml` 锁定，应使用 frozen lockfile 安装以保证可复现性。
