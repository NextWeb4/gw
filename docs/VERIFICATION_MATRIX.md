...
| Windows v0.1.1 预发布包 | `pnpm build:desktop:win:x64`、解包程序启动冒烟 | x64 已通过 | `HxHwang-Gw-0.1.1-x64-setup.exe` 为 100,944,547 字节，SHA-256 `E28CA7739CBC6B209BAD60FAD4F8B5AA7922F49DC93DF55940CF296AFB3F5803`；新解包程序启动后观察到 4 个进程并全部退出。arm64/Linux 正式产物等待 `v0.1.1` 标签的 Actions 矩阵，不提前标记完成 |
...
| GitHub Release | `v*` 标签触发桌面矩阵、Debian 门禁、校验清单和发布任务 | v0.1.0 已正式发布 | [Release](https://github.com/NextWeb4/hxhwang-gw/releases/tag/v0.1.0) 不是草稿或预发布版本，标签指向 `de69d1bf8dc7c9e9bebbcece8b9950b12483f819`；[run 30094346043](https://github.com/NextWeb4/hxhwang-gw/actions/runs/30094346043) 已成功完成并发布六个安装文件及 `SHA256SUMS.txt`，清单中的六个 SHA-256 均与 GitHub 资产摘要一致；具体校验值见 `RELEASE_NOTES.md` |
| 外部商业参考站 | 浏览器审查页面、用户协议、隐私政策和 robots | 已完成 | 仅记录 `docs/REFERENCE_AUDIT.md`；未加入 `content/sources/allowlist.yaml`，未复制其内容 |
| GitHub Pages 正式部署 | GitHub Actions / Pages 实际发布 | 已上线 | 公有客户端与私有服务端仓库已建立并推送；最新记录的 Pages Actions run [`30094067560`](https://github.com/NextWeb4/hxhwang-gw/actions/runs/30094067560) 已成功完成。[线上站点](https://nextweb4.github.io/hxhwang-gw/) 返回 HTTP 200；桌面 1440x900 与移动 390x844 实际浏览器验收均显示署名，无横向溢出、控制台错误、页面错误或第三方请求，构建资源未发现 PAT、模型密钥、私有 API 地址或嵌入凭据 |
