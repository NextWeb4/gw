...
| 客户端 lint / 单元测试 / format | `pnpm lint`、`pnpm test`、`pnpm format:check` | 已通过 | 2026-07-24 回归通过；覆盖领域、同步客户端、同步合并、迁移、文档、本地数据、2 项 Electron 开发地址策略、5 项来源抓取策略和 2 项工作流产物隔离契约；同步客户端 6 项、Web 服务层 2 项、迁移 focused 回归 5 项、文档导出 3 项均通过 |
...
| GitHub Release | `v*` 标签触发桌面矩阵、Debian 门禁、校验清单和发布任务 | 已配置，尚未实际运行 | 发布任务要求 Windows x64/arm64、AppImage x86_64/arm64、DEB amd64/arm64 六类产物齐全，并生成 `SHA256SUMS.txt`；使用 runner 自带 `gh` 与 Actions 内置令牌；远端已建立，但尚未创建发布标签或手动触发桌面工作流，不能标记已发布 |
