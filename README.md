...
![Version](https://img.shields.io/badge/version-0.6.8-0969da?style=flat-square)
...
The current release is `0.6.8`. Internet-direct, loopback-relay, intranet, and desktop AI generation now share one latest-request lifecycle: the submit button exposes a busy state, duplicate paid requests are blocked, and users can stop waiting without losing the last successful result. Browser transports receive an `AbortSignal`; late desktop IPC responses and stale history writes are rejected. Every success, failure, cancellation, or relevant input change requires a fresh per-request confirmation. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.6.8`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
