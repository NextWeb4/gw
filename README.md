...
![Version](https://img.shields.io/badge/version-0.7.1-0969da?style=flat-square)
...
The current release is `0.7.1`. The task, meeting, document, field-activity, seal, and material ledgers now share one local filter-and-sort control. Each ledger can combine keyword search, a module-specific field filter, and date/title ordering, while showing visible versus total records and an explicit reset action. View state survives module navigation for the current session but is not persisted across reloads. All transformations operate only on arrays already loaded into React, make no network or IndexedDB calls, and material balances still use the complete movement ledger. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
| --- | --- |
| Work management | Editable task, meeting, document, field-activity, seal, and material ledgers with a reusable people/organization directory, stages, summaries, attachments, per-ledger keyword/field filters and sorting, and local global search |
...
The repository version is `0.7.1`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
