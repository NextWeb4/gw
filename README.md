...
![Version](https://img.shields.io/badge/version-0.7.25-0969da?style=flat-square)
...
The current release is `0.7.25`. The task page offers session-only list, board and deadline-timeline views over the same filtered task projection. The timeline groups valid deadlines into overdue, today, the next seven days, later dates and unscheduled work, while reusing the existing read-only detail lane. Views are not persisted or synchronized, and no view provides drag-and-drop, inline status mutation or bulk writes. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.7.25`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
