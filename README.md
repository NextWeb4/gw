...
![Version](https://img.shields.io/badge/version-0.7.24-0969da?style=flat-square)
...
The current release is `0.7.24`. The task page now offers a session-only list/board switch. The board derives four fixed status columns from the current filtered and sorted task projection, shows deadline, assignee, category, checklist progress and relation counts, and reuses the existing read-only detail lane. The mode is not persisted or synchronized, and the board has no drag-and-drop, inline status mutation or bulk write path. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
| Local data | IndexedDB-backed repositories, snapshots including local document revisions and reference-only starred records, attachment references, searchable AI history, six-ledger soft deletion and restore, minimal sync tombstones, and explicit export operations |
...
The repository version is `0.7.24`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
