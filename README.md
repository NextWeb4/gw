...
![Version](https://img.shields.io/badge/version-0.7.23-0969da?style=flat-square)
...
The current release is `0.7.23`. Tasks now support an embedded checklist with explicit-save editing, completion progress in the ledger and read-only detail, checklist-text search, fixed-whitelist CSV/summary/comparison output, legacy snapshot normalization, and completion reset when copying a similar task. It uses the existing task record and persistence path, never changes task status automatically, and adds no network behavior. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.7.23`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
