...
![Version](https://img.shields.io/badge/version-0.7.20-0969da?style=flat-square)
...
The current release is `0.7.20`. Active records in the six ledgers can now be compared side by side in a read-only native dialog. The target stays in the current session, uses a fixed field whitelist, supports local search and differences-only mode, and never enters IndexedDB, snapshots, private synchronization, AI material, IPC, URLs, or network requests. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.7.20`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
