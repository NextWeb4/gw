...
![Version](https://img.shields.io/badge/version-0.6.7-0969da?style=flat-square)
...
The current release is `0.6.7`. Internet-direct, loopback-relay, and intranet model catalogs now support explicit refresh and stop-waiting controls. A refresh keeps the last successful catalog usable until a new response succeeds; cancellation or transient failure no longer clears the current choice. Successful refreshes preserve provider order and retain the selected model when it still exists. Browser requests are actively aborted, while late desktop IPC results remain rejected. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.6.7`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
