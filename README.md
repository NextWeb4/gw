...
![Version](https://img.shields.io/badge/version-0.7.10-0969da?style=flat-square)
...
The current release is `0.7.10`. Each of the six business ledgers can export its current keyword-filtered, field-filtered, and sorted visible result as CSV. Exports use fixed business-field allowlists, UTF-8 BOM, CRLF, attachment counts instead of attachment identifiers, and spreadsheet-formula neutralization. The action does not read a second database, mutate records, persist view state, or use the network. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.7.10`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
