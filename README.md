...
![Version](https://img.shields.io/badge/version-0.6.5-0969da?style=flat-square)
...
The current release is `0.6.5`. The loopback relay administration page can now test each saved provider explicitly before it is used from Pages, showing total latency, model count/preview, and a sanitized authentication/path attempt chain. Disabled providers can be tested without publishing them, stale results are discarded after edits, and diagnostics never change the encrypted revision or enter the Pages protocol. AI consent, redaction, session-only secrets, and explicit network triggers are unchanged. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.6.5`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
