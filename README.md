...
![Version](https://img.shields.io/badge/version-0.6.6-0969da?style=flat-square)
...
The current release is `0.6.6`. Internet-direct, loopback-relay, and intranet model catalogs now gain local keyword filtering above eight entries while preserving the current selection when no model matches. Model discovery exposes a busy state, and late responses cannot overwrite newer provider, endpoint, key, session, or relay-site configuration. Filtering performs no network request, persistence, or reordering. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.6.6`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
