...
![Version](https://img.shields.io/badge/version-0.7.9-0969da?style=flat-square)
...
The current release is `0.7.9`. Each of the six business ledgers now exposes previous and next controls in its read-only detail panel, following the exact current filtered and sorted order. The position indicator uses the visible count, the controls stop at the first and last record, and stepping reuses the existing selection, mobile positioning, and session-only recent-record path without persistence, writes, or network traffic. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.7.9`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
