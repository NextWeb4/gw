...
![Version](https://img.shields.io/badge/version-0.7.18-0969da?style=flat-square)
...
The current release is `0.7.18`. The bounded, session-only cross-module history now includes a keyboard- and touch-accessible visit list for jumping directly to any retained entry. Jumps only move the cursor and reuse the original record-opening path; they never enter IndexedDB, snapshots, private synchronization, AI material, IPC, URLs, or network requests. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.7.18`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
