...
![Version](https://img.shields.io/badge/version-0.7.0-0969da?style=flat-square)
...
The current release is `0.7.0`. A local global-search palette in the top bar, also available with `Ctrl/Cmd+K`, now locates all fifteen navigation modules and records from the six loaded business ledgers. Results are grouped and keyboard accessible, and record selection reuses the existing ledger and read-only detail path. Search does not make network requests, persist queries, or index API keys, access codes, relay passwords, unredacted AI material, or attachment bodies. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
| --- | --- |
| Work management | Editable task, meeting, document, field-activity, seal, and material ledgers with a reusable people/organization directory, stages, summaries, attachments, per-ledger search, and local global search |
...
The repository version is `0.7.0`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
