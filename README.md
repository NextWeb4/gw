...
![Version](https://img.shields.io/badge/version-0.6.9-0969da?style=flat-square)
...
The current release is `0.6.9`. All six business editors now expose a visible unsaved state and use one guarded close path for Cancel, the close icon, backdrop clicks, Escape, and browser unload. Clean drawers still close immediately; dirty drawers require confirmation, staged attachments remain memory-only, and attachment reads that finish after a confirmed discard cannot reopen or mutate a replaced editor. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.6.9`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
