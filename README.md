...
![Version](https://img.shields.io/badge/version-0.7.26-0969da?style=flat-square)
...
The current release is `0.7.26`. Global search treats whitespace-separated query terms as independent required filters across titles, supported ledger fields and aliases, while preserving cmdk accessibility and ordering. Search remains local/session-only with no persistence or networking. See [`RELEASE_NOTES.md`](RELEASE_NOTES.md) for details.
...
The repository version is `0.7.26`. Dependencies are locked by `pnpm-lock.yaml`; use the frozen lockfile for reproducible installs.
