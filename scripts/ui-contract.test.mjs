...
test('business drawers share one unsaved-change guard without persisting staged attachments', async () => {
  ...
  assert.match(app, /event\.key === 'Escape'[^}]*onClose\(\)/, 'drawers must support Escape through the same guarded close path');
  assert.match(app, /event\.key !== 'Tab'/, 'modal drawers must keep Tab navigation inside the active editor');
  assert.match(app, /returnFocusTarget\?\.isConnected/, 'drawers must restore focus to the still-connected opener after closing');
  assert.match(app, /\.drawer-body input:not/, 'drawers must initially focus the first visible business control rather than the header close button');
...
test('global search stays local-only and reuses accessible navigation paths', async () => {
  ...
  assert.match(globalSearch, /kind: 'navigation' \| 'record' \| 'create'/, 'the palette item contract must distinguish quick-create commands from navigation and records');
  assert.match(globalSearch, /item\.kind === 'create'[^\n]*'新建'/, 'quick-create commands must have a visible kind label');
  assert.match(globalSearch, /onSelect=\{\(\) => onSelectItem\(item\)\}/, 'selection must delegate close and activation sequencing to the app');
  ...
  assert.match(app, /id: 'create', label: '快速新建'/, 'the palette must expose one dedicated quick-create command group');
  assert.equal((app.match(/kind: 'create'/g) || []).length, 6, 'the quick-create group must cover all six editable business record types');
  assert.match(app, /if \(item\.kind === 'create'\)/, 'quick-create selection must use an explicit action branch');
  assert.match(app, /openTaskEditor\(emptyTask\(\)\)/, 'task commands must reuse the original task editor and empty draft factory');
  assert.match(app, /openMeetingEditor\(emptyMeeting\(\)\)/, 'meeting commands must reuse the original meeting editor and empty draft factory');
  assert.match(app, /openDocumentEditor\(emptyDocument\(\)\)/, 'document commands must reuse the original document editor and empty draft factory');
  assert.match(app, /openResearchEditor\(emptyResearch\(\)\)/, 'research commands must reuse the original research editor and empty draft factory');
