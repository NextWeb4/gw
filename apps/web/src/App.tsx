...
import {
  ...
  Columns3, GitCompareArrows, History, LayoutList, ListFilter, Menu, Orbit, Package, PanelLeftClose, PanelLeftOpen, PanelRightClose, PanelRightOpen, Pencil, Plus, RefreshCw, RotateCcw, Save, Search, Server, ShieldCheck, Sparkles, Star, Stamp, Trash2, Upload, UsersRound, WandSparkles, X
} from 'lucide-react';
import {
...
import { taskTimelineToday } from './task-timeline';
import type { TaskDisplayMode } from './task-board';
...
interface BusinessVisitNavigation {
  ...
  entries: BusinessVisitNavigationTarget[];
  position: number;
  total: number;
}
...
const emptyDocument = (): OfficialDocument => ({
  ...
  fromUnit: '', fileCategory: '', workCategory: '', handler: '', sendScope: '', receiptStatus: '待登记', remark: '', relatedTaskIds: [], files: [], createdAt: nowIso(), updatedAt: nowIso()
});
...
function App() {
  ...
  const [taskDisplayMode, setTaskDisplayMode] = useState<TaskDisplayMode>('list');
  ...
  const [copyDraft, setCopyDraft] = useState<{ kind: BusinessDetail['kind']; id: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const keepNarrowDetailExpanded = () => { if (!wideLayout.matches) setDetailCollapsed(false); };
    keepNarrowDetailExpanded();
    wideLayout.addEventListener('change', keepNarrowDetailExpanded);
  ...
  const filteredResearches = useMemo(() => deriveLedgerRecords('researches', researches, ledgerViews.researches), [researches, ledgerViews.researches]);
  const filteredSeals = useMemo(() => deriveLedgerRecords('seals', seals, ledgerViews.seals), [seals, ledgerViews.seals]);
  const filteredMaterials = useMemo(() => deriveLedgerRecords('materials', materials, ledgerViews.materials), [materials, ledgerViews.materials]);
  const taskFilterOptions = useMemo(() => getLedgerFilterOptions('tasks', tasks), [tasks]);
  const meetingFilterOptions = useMemo(() => getLedgerFilterOptions('meetings', meetings), [meetings]);
  const documentFilterOptions = useMemo(() => getLedgerFilterOptions('documents', documents), [documents]);
  ...
  const visitNavigation = useMemo<BusinessVisitNavigation>(() => {
    ...
    const entries = recordVisitHistory.entries.map(resolve).filter((entry): entry is BusinessVisitNavigationTarget => Boolean(entry));
    return { previous: resolve(navigation.previous), next: resolve(navigation.next), entries, position: navigation.position, total: navigation.total };
  ...
  const finishBusinessEditorSave = async (kind: BusinessDetail['kind'], tabId: BusinessTab, id: string, closeEditor: () => void) => {
    const copied = copyDraftMatches(kind, id);
    setCopyDraft(null);
    closeEditor();
    await reload();
    if (copied) {
      resetLedgerView(tabId);
      setSelectedBusinessRecord({ tab: tabId, id });
    }
  };
  ...
  const saveMeeting = async (meeting: MeetingRecord) => {
    ...
    await finishBusinessEditorSave('meeting', 'meetings', saved.id, () => setMeetingEditor(null)); setToast('会议记录已保存');
  };
  const saveDocument = async (doc: OfficialDocument) => {
    ...
    const saved = { ...doc, title: doc.title.trim(), relatedTaskIds: normalizeRelatedRecordIds(doc.relatedTaskIds), updatedAt: nowIso() };
    if (!await persistEditableRecord('document', saved.id, saved)) return;
    await persistDirectory([saved.handler], [saved.fromUnit]);
    await finishBusinessEditorSave('document', 'documents', saved.id, () => setDocumentEditor(null)); setToast('文件登记已保存，人员和单位已加入常用项');
  };
  const saveResearch = async (research: ResearchRecord) => {
    ...
    await finishBusinessEditorSave('research', 'researches', saved.id, () => setResearchEditor(null)); setToast('外出活动已保存');
  };
  const saveSeal = async (seal: SealRecord) => {
    ...
    await finishBusinessEditorSave('seal', 'seals', saved.id, () => setSealEditor(null)); setToast('用章记录已保存');
  };
  const saveMaterial = async (material: MaterialRecord) => {
    ...
    await persistDirectory([saved.handler], [saved.fromUnit]);
    await finishBusinessEditorSave('material', 'materials', saved.id, () => setMaterialEditor(null)); setToast('物资收发记录已保存');
  };
  ...
  const copyArchiveToEditable = (record: ArchiveRecord) => {
    ...
    if (record.type === 'meeting') {
      ...
      openResearchEditor({
        ...
        participants: legacyText(payload, 'participants'), summary: legacyText(payload, 'summary'), achievements: '', remark: legacyText(payload, 'remark'),
  ...
  const taskRelationCounts = useMemo(() => new Map(tasks.map((task) => [task.id, relatedDocumentsForTask(task.id, documents).length])), [tasks, documents]);
  const documentRelationCounts = useMemo(() => new Map(documents.map((document) => [document.id, relatedTasksForDocument(document, tasks).length])), [documents, tasks]);
  ...
  const duplicateBusinessDetail = (detail: BusinessDetail) => {
    ...
    if (detail.kind === 'document') openDocumentEditor(duplicateBusinessRecord('document', detail.record), true);
    if (detail.kind === 'research') openResearchEditor(duplicateBusinessRecord('research', detail.record), true);
    if (detail.kind === 'seal') openSealEditor(duplicateBusinessRecord('seal', detail.record), true);
    if (detail.kind === 'material') openMaterialEditor(duplicateBusinessRecord('material', detail.record), true);
  };
  const copyBusinessRecordSummary = async (detail: BusinessDetail) => {
    try {
      const summary = buildBusinessRecordSummary(detail.kind, detail.record as never, { tasks, documents });
      if (!navigator.clipboard?.writeText) throw new Error('clipboard unavailable');
      await navigator.clipboard.writeText(summary);
      setToast('记录摘要已复制，不会自动发送');
    } catch {
  ...
  const editBusinessDetail = (detail: BusinessDetail) => {
    ...
    if (detail.kind === 'document') openDocumentEditor(detail.record);
    if (detail.kind === 'research') openResearchEditor(detail.record);
  ...
  const jumpRecordVisitHistoryEntry = (target: RecordVisit<BusinessTab>) => {
    const jumped = jumpRecordVisitHistory(recordVisitHistoryRef.current, target);
    if (!jumped.target) return;
    replaceRecordVisitHistory(jumped.history);
    openBusinessRecord(jumped.target.tab, jumped.target.id, false);
  };
  useEffect(() => {
  ...
  const exportLedgerCsv = (createFile: () => LedgerCsvFile) => {
    try {
      ...
      setToast(`已导出${file.label}当前结果（${file.rowCount} 条）`);
    } catch {
      setToast('当前结果导出失败，请重试');
    }
  };
  ...
  const renderContent = () => {
    ...
    if (tab === 'documents') return <DocumentView documents={filteredDocuments} totalCount={documents.length} view={ledgerViews.documents} filterOptions={documentFilterOptions} onViewChange={(patch) => updateLedgerView('documents', patch)} onReset={() => resetLedgerView('documents')} onExport={() => exportLedgerCsv(() => buildLedgerCsv('documents', filteredDocuments, { date: localDateInput(new Date()), tasks }))} selectedId={selectedDocumentId} onSelect={(id) => selectBusinessRecord('documents', id)} attachments={attachments} relationCounts={documentRelationCounts} onNew={() => openDocumentEditor(emptyDocument())} onEdit={(document) => openDocumentEditor(document)} onDelete={deleteDocument} />;
    if (tab === 'researches') return <ResearchView researches={filteredResearches} totalCount={researches.length} view={ledgerViews.researches} filterOptions={researchFilterOptions} onViewChange={(patch) => updateLedgerView('researches', patch)} onReset={() => resetLedgerView('researches')} onExport={() => exportLedgerCsv(() => buildLedgerCsv('researches', filteredResearches, { date: localDateInput(new Date()) }))} selectedId={selectedResearchId} onSelect={(id) => selectBusinessRecord('researches', id)} onNew={() => openResearchEditor(emptyResearch())} onEdit={(research) => openResearchEditor(research)} onDelete={deleteResearch} />;
...
interface LedgerControlledViewProps {
  totalCount: number;
  view: LedgerViewState;
  filterOptions: readonly LedgerViewOption[];
  onViewChange: (patch: Partial<LedgerViewState>) => void;
  onReset: () => void;
  onExport: () => void;
}

function LedgerViewControls({ label, placeholder, countLabel, visibleCount, totalCount, view, filterOptions, sortOptions, onViewChange, onReset, onExport }: {
...
function TaskView({ tasks, totalCount, view, filterOptions, onViewChange, onReset, onExport, mode, onModeChange, selectedId, onSelect, attachments, relationCounts, categoryTints, onNew, onEdit, onDelete }: { tasks: Task[]; attachments: Attachment[]; relationCounts: ReadonlyMap<string, number>; categoryTints: ReadonlyMap<string, CategoryTint>; mode: TaskDisplayMode; onModeChange: (mode: TaskDisplayMode) => void; onNew: () => void; onEdit: (task: Task) => void; onDelete: (id: string) => void } & LedgerControlledViewProps & SelectableViewProps) {
  return <><PageHeading eyebrow="事务管理" title="任务管理" detail="把交办、进度、检查清单、配合单位和工作小结放在同一条记录里。" action={<button className="primary-button" onClick={onNew}><Plus size={16} />新建任务</button>} /><LedgerViewControls label="任务管理" placeholder="搜索任务、清单、类目或交办人" countLabel="条任务" visibleCount={tasks.length} totalCount={totalCount} view={view} filterOptions={filterOptions} sortOptions={LEDGER_SORT_OPTIONS.tasks} onViewChange={onViewChange} onReset={onReset} onExport={onExport} /><div className="task-display-toolbar"><div className="task-display-mode" role="group" aria-label="任务展示方式"><button type="button" aria-pressed={mode === 'list'} title="切换到任务列表" onClick={() => onModeChange('list')}><LayoutList size={15} />列表</button><button type="button" aria-pressed={mode === 'board'} title="切换到状态看板" onClick={() => onModeChange('board')}><Columns3 size={15} />看板</button><button type="button" aria-pressed={mode === 'timeline'} title="切换到截止时间轴" onClick={() => onModeChange('timeline')}><CalendarRange size={15} />时间轴</button></div></div>{mode === 'list' ? <section className="panel table-panel"><div className="table-head task-columns"><span>任务</span><span>来源 / 类目</span><span>截止日期</span><span>状态</span><span /></div>{tasks.map((task) => { const linkedDocumentCount = relationCounts.get(task.id) || 0; const checklistProgress = taskChecklistProgress(task.checklist); return <div className={`table-row task-columns selectable-row ${selectedId === task.id ? 'selected' : ''}`} key={task.id} tabIndex={0} title={`查看任务详情：${task.name}`} onClick={() => onSelect(task.id)} onKeyDown={(event) => selectRecordOnKeyboard(event, () => onSelect(task.id))}><div className="row-title"><strong>{task.name}</strong><small>{task.assigner || '未指定交办人'} · {task.workSummary || '尚无工作小结'}{checklistProgress.total ? ` · 检查清单 ${checklistProgress.completed}/${checklistProgress.total}` : ''} · 附件 {task.files.length}{linkedDocumentCount ? ` · 关联文件 ${linkedDocumentCount}` : ''}</small></div><span className="muted-cell">{task.source || '其他'}<br /><span className="category-chip"><span className={`category-dot tint-${resolveCategoryTint(task.category, categoryTints)}`} aria-hidden="true" />{task.category || '未分类'}</span></span><span className="date-cell">{task.deadline || '—'}</span><StatusPill status={task.status} /><RowActions editTitle="编辑任务" deleteTitle="删除任务" onEdit={() => onEdit(task)} onDelete={() => onDelete(task.id)} /></div>; })}{!tasks.length && <EmptyState text="没有匹配的任务" />}</section> : mode === 'board' ? <TaskBoard tasks={tasks} selectedId={selectedId} relationCounts={relationCounts} categoryTints={categoryTints} onSelect={onSelect} /> : <TaskTimeline tasks={tasks} today={taskTimelineToday()} selectedId={selectedId} relationCounts={relationCounts} categoryTints={categoryTints} onSelect={onSelect} />}<AttachmentHint count={attachments.length} /></>;
}
...
function MeetingView({ meetings, totalCount, view, filterOptions, onViewChange, onReset, onExport, selectedId, onSelect, onNew, onEdit, onDelete }: { meetings: MeetingRecord[]; onNew: () => void; onEdit: (meeting: MeetingRecord) => void; onDelete: (id: string) => void } & LedgerControlledViewProps & SelectableViewProps) {
  return <><PageHeading eyebrow="会议台账" title="会议管理" detail="记录通知对象、会议时间、地点和本机附件。" action={<button className="primary-button" onClick={onNew}><Plus size={16} />新建会议</button>} /><LedgerViewControls label="会议管理" placeholder="搜索主题、对象、人员或地点" countLabel="场会议" visibleCount={meetings.length} totalCount={totalCount} view={view} filterOptions={filterOptions} sortOptions={LEDGER_SORT_OPTIONS.meetings} onViewChange={onViewChange} onReset={onReset} onExport={onExport} /><section className="panel table-panel"><div className="table-head business-columns"><span>会议主题</span><span>会议 / 通知时间</span><span>对象与接收方</span><span>地点</span><span /></div>{meetings.map((meeting) => <div className={`table-row business-columns selectable-row ${selectedId === meeting.id ? 'selected' : ''}`} key={meeting.id} tabIndex={0} title={`查看会议详情：${meeting.subject}`} onClick={() => onSelect(meeting.id)} onKeyDown={(event) => selectRecordOnKeyboard(event, () => onSelect(meeting.id))}><div className="row-title"><strong>{meeting.subject}</strong><small>{meeting.remark || '无备注'} · 附件 {meeting.files.length}</small></div><span className="muted-cell">{meeting.meetingTime ? meeting.meetingTime.replace('T', ' ') : '未设会议时间'}<br />通知 {meeting.notifyTime || '—'}</span><span className="muted-cell">{meeting.sendTo || '未填写发送对象'}<br />{meeting.receiver || '未填写接收方'}</span><span className="muted-cell">{meeting.location || '未填写'}</span><RowActions editTitle="编辑会议" deleteTitle="删除会议" onEdit={() => onEdit(meeting)} onDelete={() => onDelete(meeting.id)} /></div>)}{!meetings.length && <EmptyState text="没有匹配的会议记录" />}</section></>;
}

function DocumentView({ documents, totalCount, view, filterOptions, onViewChange, onReset, onExport, selectedId, onSelect, attachments, relationCounts, onNew, onEdit, onDelete }: { documents: OfficialDocument[]; attachments: Attachment[]; relationCounts: ReadonlyMap<string, number>; onNew: () => void; onEdit: (doc: OfficialDocument) => void; onDelete: (id: string) => void } & LedgerControlledViewProps & SelectableViewProps) {
  return <><PageHeading eyebrow="文件台账" title="文件收发" detail="登记文件来源、文号、承办人和关联工作，附件留在本地。" action={<button className="primary-button" onClick={onNew}><Plus size={16} />登记文件</button>} /><LedgerViewControls label="文件收发" placeholder="搜索标题、文号或来源单位" countLabel="份文件" visibleCount={documents.length} totalCount={totalCount} view={view} filterOptions={filterOptions} sortOptions={LEDGER_SORT_OPTIONS.documents} onViewChange={onViewChange} onReset={onReset} onExport={onExport} /><section className="panel table-panel"><div className="table-head doc-columns"><span>文件标题</span><span>文号 / 类型</span><span>来源单位</span><span>登记状态</span><span /></div>{documents.map((doc) => { const linkedTaskCount = relationCounts.get(doc.id) || 0; return <div className={`table-row doc-columns selectable-row ${selectedId === doc.id ? 'selected' : ''}`} key={doc.id} tabIndex={0} title={`查看文件详情：${doc.title}`} onClick={() => onSelect(doc.id)} onKeyDown={(event) => selectRecordOnKeyboard(event, () => onSelect(doc.id))}><div className="row-title"><strong>{doc.title}</strong><small>{doc.docDate || '未设日期'} · {doc.securityLevel || '未分级'} · 附件 {doc.files.length}{linkedTaskCount ? ` · 关联任务 ${linkedTaskCount}` : ''}</small></div><span className="muted-cell">{doc.code || '无文号'}<br />{doc.docType}</span><span className="muted-cell">{doc.fromUnit || '未填写'}</span><span className="status-pill neutral">{doc.receiptStatus || '待登记'}</span><RowActions editTitle="编辑文件" deleteTitle="删除文件" onEdit={() => onEdit(doc)} onDelete={() => onDelete(doc.id)} /></div>; })}{!documents.length && <EmptyState text="还没有登记文件" />}</section><AttachmentHint count={attachments.length} /></>;
}

function ResearchView({ researches, totalCount, view, filterOptions, onViewChange, onReset, onExport, selectedId, onSelect, onNew, onEdit, onDelete }: { researches: ResearchRecord[]; onNew: () => void; onEdit: (research: ResearchRecord) => void; onDelete: (id: string) => void } & LedgerControlledViewProps & SelectableViewProps) {
  return <><PageHeading eyebrow="外勤台账" title="外出活动" detail="记录调研、会议、慰问、来访和活动成果。" action={<button className="primary-button" onClick={onNew}><Plus size={16} />新建外出活动</button>} /><LedgerViewControls label="外出活动" placeholder="搜索主题、类型、人员、地点或摘要" countLabel="项活动" visibleCount={researches.length} totalCount={totalCount} view={view} filterOptions={filterOptions} sortOptions={LEDGER_SORT_OPTIONS.researches} onViewChange={onViewChange} onReset={onReset} onExport={onExport} /><section className="panel table-panel"><div className="table-head business-columns"><span>活动主题</span><span>日期 / 类型</span><span>参与人员</span><span>地点 / 用车</span><span /></div>{researches.map((research) => <div className={`table-row business-columns selectable-row ${selectedId === research.id ? 'selected' : ''}`} key={research.id} tabIndex={0} title={`查看外出活动详情：${research.subject}`} onClick={() => onSelect(research.id)} onKeyDown={(event) => selectRecordOnKeyboard(event, () => onSelect(research.id))}><div className="row-title"><strong>{research.subject}</strong><small>{research.achievements || research.summary || '尚无成果记录'} · 附件 {research.files.length}</small></div><span className="muted-cell">{research.researchTime || '未设日期'}<br />{research.direction}</span><span className="muted-cell">{research.participants || '未填写'}</span><span className="muted-cell">{research.location || '未填写'}<br />用车：{research.useCar || '未选择'}</span><RowActions editTitle="编辑外出活动" deleteTitle="删除外出活动" onEdit={() => onEdit(research)} onDelete={() => onDelete(research.id)} /></div>)}{!researches.length && <EmptyState text="没有匹配的外出活动" />}</section></>;
}

function SealView({ seals, totalCount, view, filterOptions, onViewChange, onReset, onExport, selectedId, onSelect, onNew, onEdit, onDelete }: { seals: SealRecord[]; onNew: () => void; onEdit: (seal: SealRecord) => void; onDelete: (id: string) => void } & LedgerControlledViewProps & SelectableViewProps) {
  return <><PageHeading eyebrow="审批台账" title="用章管理" detail="登记用章文件、经办人、审批人和佐证附件。" action={<button className="primary-button" onClick={onNew}><Plus size={16} />新建用章记录</button>} /><LedgerViewControls label="用章管理" placeholder="搜索文件、用章人、审批人或类型" countLabel="次用章" visibleCount={seals.length} totalCount={totalCount} view={view} filterOptions={filterOptions} sortOptions={LEDGER_SORT_OPTIONS.seals} onViewChange={onViewChange} onReset={onReset} onExport={onExport} /><section className="panel table-panel"><div className="table-head business-columns"><span>所盖文件</span><span>日期 / 类型</span><span>用章人</span><span>审批人</span><span /></div>{seals.map((seal) => <div className={`table-row business-columns selectable-row ${selectedId === seal.id ? 'selected' : ''}`} key={seal.id} tabIndex={0} title={`查看用章详情：${seal.docName}`} onClick={() => onSelect(seal.id)} onKeyDown={(event) => selectRecordOnKeyboard(event, () => onSelect(seal.id))}><div className="row-title"><strong>{seal.docName}</strong><small>{seal.remark || '无备注'} · 附件 {seal.files.length}</small></div><span className="muted-cell">{seal.sealTime || '未设日期'}<br />{seal.docType || '未分类'}</span><span className="muted-cell">{seal.userName}</span><span className="muted-cell">{seal.approver}</span><RowActions editTitle="编辑用章记录" deleteTitle="删除用章记录" onEdit={() => onEdit(seal)} onDelete={() => onDelete(seal.id)} /></div>)}{!seals.length && <EmptyState text="没有匹配的用章记录" />}</section></>;
}

function MaterialView({ materials, allMaterials, totalCount, view, filterOptions, onViewChange, onReset, onExport, selectedId, onSelect, onNew, onEdit, onDelete }: { materials: MaterialRecord[]; allMaterials: MaterialRecord[]; onNew: () => void; onEdit: (material: MaterialRecord) => void; onDelete: (id: string) => void } & LedgerControlledViewProps & SelectableViewProps) {
  const balances = calculateMaterialStock(allMaterials);
  return <><PageHeading eyebrow="保障台账" title="物资收发" detail="按物资名称和规格汇总入库、领用与当前账面库存。" action={<button className="primary-button" onClick={onNew}><Plus size={16} />新建物资记录</button>} /><LedgerViewControls label="物资收发" placeholder="搜索物资、规格、经手人或单位" countLabel="笔收发" visibleCount={materials.length} totalCount={totalCount} view={view} filterOptions={filterOptions} sortOptions={LEDGER_SORT_OPTIONS.materials} onViewChange={onViewChange} onReset={onReset} onExport={onExport} /><section className="panel table-panel"><div className="table-head business-columns"><span>物资名称</span><span>收发 / 数量</span><span>经手信息</span><span>账面库存</span><span /></div>{materials.map((material) => <div className={`table-row business-columns selectable-row ${selectedId === material.id ? 'selected' : ''}`} key={material.id} tabIndex={0} title={`查看物资详情：${material.materialName}`} onClick={() => onSelect(material.id)} onKeyDown={(event) => selectRecordOnKeyboard(event, () => onSelect(material.id))}><div className="row-title"><strong>{material.materialName}</strong><small>{material.spec || '无规格'} · {material.fromUnit || '未填写来源/领用单位'} · 附件 {material.files.length}</small></div><span className={`status-pill ${material.type === 'in' ? 'done' : 'pending'}`}>{material.type === 'in' ? '入库' : '领用'} {material.quantity}</span><span className="muted-cell">{material.handlerTime || '未设日期'}<br />{material.handler || '未填写经手人'}</span><span className="stock-balance">{balances.get(`${material.materialName.trim()}|${material.spec.trim()}`) || 0}</span><RowActions editTitle="编辑物资记录" deleteTitle="删除物资记录" onEdit={() => onEdit(material)} onDelete={() => onDelete(material.id)} /></div>)}{!materials.length && <EmptyState text="没有匹配的物资记录" />}</section></>;
}
...
function BusinessDetailPanel({ detail, navigation, visitNavigation, tasks, documents, attachments, panelRef, collapsed, comparisonAvailable, starred, onToggleStarred, onToggleCollapsed, onCompare, onCopySummary, onNavigateVisit, onJumpToVisit, onNavigateVisibleRecord, onOpenRelatedRecord, onBackToList, onEdit, onDuplicate }: { detail: BusinessDetail; navigation: BusinessDetailNavigation; visitNavigation: BusinessVisitNavigation; tasks: Task[]; documents: OfficialDocument[]; attachments: Attachment[]; panelRef: React.RefObject<HTMLElement | null>; collapsed: boolean; comparisonAvailable: boolean; starred: boolean; onToggleStarred: () => void; onToggleCollapsed: () => void; onCompare: () => void; onCopySummary: () => void; onNavigateVisit: (direction: 'back' | 'forward') => void; onJumpToVisit: (target: RecordVisit<BusinessTab>) => void; onNavigateVisibleRecord: (id: string) => void; onOpenRelatedRecord: (tab: BusinessTab, id: string) => void; onBackToList: () => void; onEdit: () => void; onDuplicate: () => void }) {
  ...
  const unavailableTaskLinks = detail.kind === 'document' ? Math.max(0, normalizeRelatedRecordIds(detail.record.relatedTaskIds).length - linkedTasks.length) : 0;
  const model: BusinessDetailModel = (() => {
    ...
    if (detail.kind === 'document') {
      ...
      return {
        ...
        sections: [
          { title: '关联任务', content: <RelatedRecordList kind="task" records={linkedTasks.map((task) => ({ id: task.id, title: task.name, meta: `${statusLabels[task.status]}${task.deadline ? ` · 截止 ${task.deadline}` : ''}` }))} emptyText="当前文件没有关联的 active 任务" unavailableCount={unavailableTaskLinks} onOpen={(id) => onOpenRelatedRecord('tasks', id)} /> },
          { title: '备注', content: show(document.remark) }
        ]
      };
    }
  ...
  useEffect(() => {
    if (visitMenuOpen && !visitNavigation.entries.length) setVisitMenuOpen(false);
  }, [visitMenuOpen, visitNavigation.entries.length]);
  const moveVisitMenuFocus = (index: number, event: React.KeyboardEvent<HTMLButtonElement>) => {
    if (!visitNavigation.entries.length || !['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(event.key)) return;
    event.preventDefault();
    const next = event.key === 'Home' ? 0 : event.key === 'End' ? visitNavigation.entries.length - 1 : (index + (event.key === 'ArrowDown' ? 1 : -1) + visitNavigation.entries.length) % visitNavigation.entries.length;
    visitMenuOptionRefs.current[next]?.focus();
  };
  ...
  if (collapsed) {
    return <aside className="panel business-detail-panel detail-panel-collapsed" aria-label="记录详情（已收起）" ref={panelRef}>
      <div className="detail-collapsed-rail">
        <span className="detail-icon" aria-hidden="true"><Icon size={18} /></span>
        <span className="sr-only">{model.title}</span>
        <button type="button" className="icon-button detail-collapse-toggle" aria-label="展开右侧记录详情" title="展开右侧记录详情" onClick={onToggleCollapsed}><PanelRightOpen size={17} /></button>
      </div>
    </aside>;
  }
  return <aside className="panel business-detail-panel" aria-label="记录详情" ref={panelRef}>
    ...
    <div className="detail-visit-navigation">
      ...
      <div className="detail-visit-steps" role="group" aria-label="跨模块访问历史">
        ...
        {visitNavigation.entries.length > 0 && <div className="detail-visit-menu" ref={visitMenuRef}>
          <button ref={visitMenuToggleRef} type="button" className="icon-button detail-visit-menu-toggle" aria-label="打开访问轨迹列表" aria-haspopup="listbox" aria-expanded={visitMenuOpen} title="打开访问轨迹列表" onClick={() => setVisitMenuOpen((open) => !open)}><History size={17} /></button>
...
function RelatedRecordList({ kind, records, emptyText, unavailableCount = 0, onOpen }: { kind: 'task' | 'document'; records: Array<{ id: string; title: string; meta: string }>; emptyText: string; unavailableCount?: number; onOpen: (id: string) => void }) {
  const label = kind === 'task' ? '任务' : '文件';
  return <div className="detail-related-list">
    {records.map((record) => <button type="button" key={record.id} aria-label={`打开关联${label}：${record.title}`} onClick={() => onOpen(record.id)}><Link2 size={14} /><span><strong>{record.title}</strong>{record.meta && <small>{record.meta}</small>}</span><ArrowUpRight size={13} /></button>)}
    {!records.length && <p>{emptyText}</p>}
    {unavailableCount > 0 && <p className="detail-related-unavailable">另有 {unavailableCount} 个关联任务当前不可用，可能已移入回收站、永久删除或不在本机。</p>}
  </div>;
}
...
function CopyDraftNotice({ detail }: { detail: string }) {
  return <div className="copy-draft-notice" role="note"><CopyPlus size={18} /><div><strong>这是未保存的新记录</strong><p>{detail} 解除附件关联不会影响原记录，只有点击保存后才会写入本机数据。</p></div></div>;
}
...
function TaskEditor({ task, isNew, forceDirty, initialImportText, returnFocusTarget, directory, attachments, attachmentBusy, partnerGroups, onSaveGroup, onDeleteGroup, onRemember, onChange, onAttach, onSave, onClose, setToast }: { task: Task; isNew: boolean; forceDirty: boolean; initialImportText: string; returnFocusTarget: HTMLElement | null; directory: ContactDirectory; attachments: Attachment[]; attachmentBusy: boolean; partnerGroups: PartnerGroup[]; onSaveGroup: (name: string, members: string[]) => Promise<void>; onDeleteGroup: (group: PartnerGroup) => Promise<void>; onRemember: (kind: 'people' | 'units', value: string) => Promise<void>; onChange: (task: Task) => void; onAttach: (files: FileList) => void; onSave: () => void; onClose: () => void; setToast: (text: string) => void }) {
  ...
  const closeGuard = useUnsavedChangesGuard({ task, importText, attachmentBusy }, onClose, forceDirty || Boolean(initialImportText));
...
function RelatedTaskField({ tasks, selectedIds, onChange }: { tasks: Task[]; selectedIds: readonly string[] | undefined; onChange: (ids: string[]) => void }) {
  const [query, setQuery] = useState('');
  const [open, setOpen] = useState(true);
  const normalizedIds = normalizeRelatedRecordIds(selectedIds);
  const selectedSet = new Set(normalizedIds);
  const activeSelected = relatedTasksForDocument({ relatedTaskIds: normalizedIds }, tasks);
  const activeSelectedIds = new Set(activeSelected.map((task) => task.id));
  const unavailableCount = Math.max(0, normalizedIds.length - activeSelected.length);
  const normalizedQuery = query.trim().toLocaleLowerCase('zh-CN');
  const visibleTasks = tasks.filter((task) => !normalizedQuery || [task.name, task.category, task.source, task.assigner, task.deadline].some((value) => value.toLocaleLowerCase('zh-CN').includes(normalizedQuery)));
  const toggle = (id: string, checked: boolean) => onChange(normalizeRelatedRecordIds(checked ? [...normalizedIds, id] : normalizedIds.filter((candidate) => candidate !== id)));
  return <details className="related-record-field" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
    <summary><span><Link2 size={15} />关联任务</span><small>{activeSelected.length} 个可用关联</small></summary>
    <div className="related-record-body">
      <p>只选择当前已加载的 active 任务；筛选和勾选均在本机完成，只有点击“保存文件”才会写入关系。</p>
      {activeSelected.length > 0 && <div className="related-record-selected" aria-label="已关联任务">{activeSelected.map((task) => <button type="button" key={task.id} aria-label={`取消关联任务：${task.name}`} onClick={() => toggle(task.id, false)}><span>{task.name}</span><X size={13} /></button>)}</div>}
      {unavailableCount > 0 && <div className="related-record-unavailable"><p>保留 {unavailableCount} 个当前不可用的弱引用；它们可能在回收站、已永久删除或来自另一设备。</p><button type="button" onClick={() => onChange(normalizedIds.filter((id) => activeSelectedIds.has(id)))}>清理不可用关联</button></div>}
      <label className="related-record-filter"><Search size={14} /><input aria-label="筛选可关联任务" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="按任务名称、类目、来源、交办人或截止日期筛选" />{query && <button type="button" className="icon-button" aria-label="清除关联任务筛选" onClick={() => setQuery('')}><X size={13} /></button>}</label>
      <div className="related-record-options" role="group" aria-label="可关联任务">
        {visibleTasks.map((task) => <label className="related-record-option" key={task.id}><input type="checkbox" aria-label={`关联任务：${task.name}`} checked={selectedSet.has(task.id)} onChange={(event) => toggle(task.id, event.target.checked)} /><span><strong>{task.name}</strong><small>{statusLabels[task.status]}{task.deadline ? ` · 截止 ${task.deadline}` : ' · 未排期'}{task.category ? ` · ${task.category}` : ''}</small></span></label>)}
        {!visibleTasks.length && <span className="related-record-empty">没有匹配的 active 任务</span>}
      </div>
    </div>
  </details>;
}

function DocumentEditor({ document, tasks, isNew, forceDirty, returnFocusTarget, directory, attachments, attachmentBusy, onRemember, onChange, onAttach, onSave, onClose }: { document: OfficialDocument; tasks: Task[]; isNew: boolean; forceDirty: boolean; returnFocusTarget: HTMLElement | null; directory: ContactDirectory; attachments: Attachment[]; attachmentBusy: boolean; onRemember: (kind: 'people' | 'units', value: string) => Promise<void>; onChange: (document: OfficialDocument) => void; onAttach: (files: FileList) => void; onSave: () => void; onClose: () => void }) {
  ...
  return <Drawer title={forceDirty ? '复制文件为新记录' : isNew ? '登记文件' : '编辑文件'} dirty={closeGuard.dirty} returnFocusTarget={returnFocusTarget} onClose={closeGuard.requestClose}>
    {forceDirty && <CopyDraftNotice detail="已保留原字段和本机附件引用，登记状态已重置为“待登记”，关联任务已清空；请重点核对成文日期与发文字号。" />}
    ...
    <RelatedTaskField tasks={tasks} selectedIds={document.relatedTaskIds} onChange={(ids) => update('relatedTaskIds', ids)} />
...
function weeklyEditableFingerprint(report: WeeklyReport) {
  return JSON.stringify({
    title: report.title, startDate: report.startDate, endDate: report.endDate, contentText: report.contentText,
    taskIds: report.taskIds, documentIds: report.documentIds, meetingIds: report.meetingIds || [], researchIds: report.researchIds || [], sealIds: report.sealIds || [], materialIds: report.materialIds || [],
  });
}

type WeeklySourceItem = { title: string; meta: string };
type WeeklySourceKind = 'task' | 'document' | 'meeting' | 'research' | 'seal' | 'material';
type WeeklySourceIndex = Record<WeeklySourceKind, Map<string, WeeklySourceItem>>;
const weeklySourcePresentation: Record<WeeklySourceKind, { label: string; tab: BusinessTab; icon: typeof ClipboardList }> = {
  task: { label: '任务', tab: 'tasks', icon: ClipboardList },
  document: { label: '文件', tab: 'documents', icon: FileText },
  meeting: { label: '会议', tab: 'meetings', icon: CalendarDays },
  research: { label: '外出', tab: 'researches', icon: MapPin },
  seal: { label: '用章', tab: 'seals', icon: Stamp },
  material: { label: '物资', tab: 'materials', icon: Package },
};

function WeeklyRelatedSources({ groups, index, onOpenRecord }: { groups: ReturnType<typeof resolveWeeklyReportRelationGroups>; index: WeeklySourceIndex; onOpenRecord: (tab: BusinessTab, id: string) => void }) {
  const visibleGroups = groups.filter((group) => group.ids.length || group.unavailableCount);
  const availableCount = groups.reduce((total, group) => total + group.ids.length, 0);
  const unavailableCount = groups.reduce((total, group) => total + group.unavailableCount, 0);
  return <section className="weekly-related-sources" aria-label="周报素材来源">
    <div className="weekly-related-heading"><span><Link2 size={14} />来源记录</span><small>{availableCount} 条可打开</small></div>
    {!visibleGroups.length && <p className="weekly-related-empty">当前日期范围没有可关联的业务记录</p>}
    {visibleGroups.map((group) => {
      const presentation = weeklySourcePresentation[group.kind];
      const Icon = presentation.icon;
      return <div className="weekly-related-group" key={group.kind}>
        <h3><span>{presentation.label}</span><small>{group.ids.length}</small></h3>
        {group.ids.map((id) => { const record = index[group.kind].get(id); return record ? <button type="button" key={id} aria-label={`打开周报来源${presentation.label}：${record.title}`} onClick={() => onOpenRecord(presentation.tab, id)}><Icon size={14} /><span><strong>{record.title}</strong>{record.meta && <small>{record.meta}</small>}</span><ArrowUpRight size={13} /></button> : null; })}
        {group.unavailableCount > 0 && <p>另有 {group.unavailableCount} 条{presentation.label}当前不可用</p>}
      </div>;
    })}
    {unavailableCount > 0 && <p className="weekly-related-notice">回收站、永久删除或不在本机的记录不会进入周报来源列表。</p>}
  </section>;
}

function WeeklyView({ tasks, meetings, documents, researches, seals, materials, reports, revisions, templates, onSave, onDelete, onDeleteRevision, onClearRevisions, onRevisionDialogOpenChange, onSaveTemplate, onDeleteTemplate, onAiAssist, onOpenRecord, setToast }: { tasks: Task[]; meetings: MeetingRecord[]; documents: OfficialDocument[]; researches: ResearchRecord[]; seals: SealRecord[]; materials: MaterialRecord[]; reports: WeeklyReport[]; revisions: DocumentRevision[]; templates: WeeklyTemplate[]; onSave: (report: WeeklyReport) => Promise<WeeklyReport>; onDelete: (id: string) => Promise<boolean>; onDeleteRevision: (revision: DocumentRevision) => Promise<void>; onClearRevisions: (id: string) => Promise<void>; onRevisionDialogOpenChange: (open: boolean) => void; onSaveTemplate: (name: string, sections: WeeklyTemplateSection[], id?: string) => Promise<string | null>; onDeleteTemplate: (template: WeeklyTemplate) => Promise<void>; onAiAssist: (request?: AiAssistRequest) => void; onOpenRecord: (tab: BusinessTab, id: string) => void; setToast: (text: string) => void }) {
  ...
  const sourceGroups = resolveWeeklyReportRelationGroups(current, { tasks, meetings, documents, researches, seals, materials });
  const sourceIndex: WeeklySourceIndex = {
    task: new Map<string, WeeklySourceItem>(tasks.map((task) => [task.id, { title: task.name, meta: `${statusLabels[task.status]}${task.deadline ? ` · 截止 ${task.deadline}` : ''}` }])),
    document: new Map<string, WeeklySourceItem>(documents.map((document) => [document.id, { title: document.title, meta: [document.code, document.docDate].filter(Boolean).join(' · ') }])),
    meeting: new Map<string, WeeklySourceItem>(meetings.map((meeting) => [meeting.id, { title: meeting.subject, meta: meeting.meetingTime ? meeting.meetingTime.replace('T', ' ') : '未设会议时间' }])),
    research: new Map<string, WeeklySourceItem>(researches.map((research) => [research.id, { title: research.subject, meta: [research.researchTime, research.location].filter(Boolean).join(' · ') }])),
    seal: new Map<string, WeeklySourceItem>(seals.map((seal) => [seal.id, { title: seal.docName, meta: [seal.sealTime, seal.docType].filter(Boolean).join(' · ') }])),
    material: new Map<string, WeeklySourceItem>(materials.map((material) => [material.id, { title: material.materialName, meta: `${material.type === 'in' ? '入库' : '领用'} · ${material.quantity}` }])),
  };
  const savedHead = current.id ? reports.find((saved) => saved.id === current.id) : undefined;
  const hasUnsavedChanges = (report !== null && (!savedHead || weeklyEditableFingerprint(savedHead) !== weeklyEditableFingerprint(current))) || (report === null && Boolean(templateId));
  const openSourceRecord = (tab: BusinessTab, id: string) => {
    if (hasUnsavedChanges && !window.confirm('当前周报有未保存修改，打开来源记录会丢失这些修改。确认继续？')) return;
    onOpenRecord(tab, id);
  };
