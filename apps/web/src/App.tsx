...
import {
  createId, nowIso, type ArchiveRecord, type Attachment, type Draft, type KnowledgePack, type MigrationReport, type OfficialDocument, type Status, type Task,
...
function App() {
  ...
  const importLegacy = async (bundle: MigrationBundle) => {
    ...
    for (const setting of bundle.settings) await putRecord('setting', setting.id, { id: setting.id, value: setting.value });
    await reload();
...
function TaskEditor({ task, attachments, canAttach, onChange, onAttach, onSave, onClose }: { task: Task; attachments: Attachment[]; canAttach: boolean; onChange: (task: Task) => void; onAttach: (files: FileList) => void; onSave: () => void; onClose: () => void }) {
  const update = (key: keyof Task, value: string) => onChange({ ...task, [key]: value });
  return <Drawer title={task.name ? '编辑任务' : '新建任务'} onClose={onClose}><Field label="任务名称" value={task.name} onChange={(v) => update('name', v)} placeholder="例如：推进年度重点工作总结" /><div className="form-grid"><SelectField label="状态" value={task.status} options={['pending', 'progress', 'done', 'overdue']} onChange={(v) => update('status', v as Status)} /><Field label="截止日期" type="date" value={task.deadline} onChange={(v) => update('deadline', v)} /></div><div className="form-grid"><Field label="工作类目" value={task.category} onChange={(v) => update('category', v)} /><Field label="交办人" value={task.assigner} onChange={(v) => update('assigner', v)} /></div><div className="form-grid"><Field label="任务来源" value={task.source} onChange={(v) => update('source', v)} /><Field label="交办日期" type="date" value={task.assignDate} onChange={(v) => update('assignDate', v)} /></div><TextArea label="工作小结" value={task.workSummary} onChange={(v) => update('workSummary', v)} placeholder="生成周报时可引用" /><TextArea label="备注" value={task.remark} onChange={(v) => update('remark', v)} /><AttachmentField ids={task.files} attachments={attachments} canAttach={canAttach} onAttach={onAttach} /><div className="drawer-actions"><button className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" onClick={onSave}><Save size={16} />保存任务</button></div></Drawer>;
}
...
function DocumentEditor({ document, attachments, canAttach, onChange, onAttach, onSave, onClose }: { document: OfficialDocument; attachments: Attachment[]; canAttach: boolean; onChange: (document: OfficialDocument) => void; onAttach: (files: FileList) => void; onSave: () => void; onClose: () => void }) {
  const update = (key: keyof OfficialDocument, value: string) => onChange({ ...document, [key]: value });
  return <Drawer title={document.title ? '编辑文件' : '登记文件'} onClose={onClose}><Field label="文件标题" value={document.title} onChange={(v) => update('title', v)} placeholder="例如：关于做好年度重点工作的通知" /><div className="form-grid"><SelectField label="文件类型" value={document.docType} options={['收文', '发文', '其他']} onChange={(v) => update('docType', v as OfficialDocument['docType'])} /><Field label="成文日期" type="date" value={document.docDate} onChange={(v) => update('docDate', v)} /></div><div className="form-grid"><Field label="发文字号" value={document.code} onChange={(v) => update('code', v)} /><Field label="来源单位" value={document.fromUnit} onChange={(v) => update('fromUnit', v)} /></div><div className="form-grid"><Field label="承办人" value={document.handler} onChange={(v) => update('handler', v)} /><SelectField label="登记状态" value={document.receiptStatus} options={['待登记', '已登记', '已办结', '归档']} onChange={(v) => update('receiptStatus', v)} /></div><div className="form-grid"><Field label="安全级别" value={document.securityLevel} onChange={(v) => update('securityLevel', v)} /><Field label="工作类目" value={document.workCategory} onChange={(v) => update('workCategory', v)} /></div><TextArea label="备注" value={document.remark} onChange={(v) => update('remark', v)} /><AttachmentField ids={document.files} attachments={attachments} canAttach={canAttach} onAttach={onAttach} /><div className="drawer-actions"><button className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" onClick={onSave}><Save size={16} />保存文件</button></div></Drawer>;
}
  ...
  return <>
    ...
    <div className="migration-grid">
      ...
      <section className="panel migration-card">
        ...
        <h2>导出本地快照</h2>
        <p>将任务、文件、草稿、历史档案和附件元数据导出为可恢复快照。</p>
...
function AboutView({ desktop, privateServices, tasks, documents, attachments, draft, onReload, setToast }: { desktop: boolean; privateServices: boolean; tasks: Task[]; documents: OfficialDocument[]; attachments: Attachment[]; draft: Draft; onReload: () => Promise<void>; setToast: (text: string) => void }) {
  ...
  const sync = async () => {
    ...
    try {
      const result = await syncPrivateWorkspace(client, { tasks, documents, drafts: [draft], attachments });
      await onReload();
