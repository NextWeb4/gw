...
import { sampleTasks, type Attachment, type Draft, type Task, type WeeklyReport } from '@hxhwang/domain';
...
describe('private workspace sync', () => {
  it('pulls newer masters, pushes local-only records and transfers attachments', async () => {
    ...
    const result = await syncPrivateWorkspace(client, { tasks: [localTask], documents: [], drafts: [draft], weeklyReports: [weeklyReport], attachments: [attachment, emptyAttachment] }, writeRecord);
  ...
  it('pushes a newer local state with the exact remote master assumption', async () => {
    ...
    await syncPrivateWorkspace(client, { tasks: [localTask], documents: [], drafts: [], weeklyReports: [], attachments: [] }, writeRecord);
