...
- 工作台新增“复制今日简报”。简报从既有工作焦点复用同一批 active 任务、会议、文件、外出、用章和物资记录，并按同一日期口径派生固定章节：逾期、今天、未来 7 天和未排期。
- 输出是确定性纯文本：首行带今日锚点日期，每条显示模块、标题和业务时间；空节保留“暂无”，无效或缺失日期归入“未排期”，不静默截断结果。
- 复制只在用户显式点击后等待本机剪贴板写入成功；失败时给出明确提示。回收站、永久删除墓碑和已完成任务不会进入待办章节，简报也不会自动发送。
- 该能力只消费当前已加载数组，不新增依赖、数据库 schema、API、IPC 或网络请求；不写 IndexedDB、localStorage、sessionStorage、快照或私有同步。
- 当前本机验证已完成：`pnpm lint`、`pnpm format:check`、`pnpm test` 通过；workspace 测试为 Desktop 9、Domain 45、Sync 27、Documents 6、Migration 5、Local data 21、Web 56、Content 5、Workflow 9、UI contract 26。工作台聚焦桌面/移动 E2E 为 2/2；`git diff --check` 通过。完整主/互联网/内网 E2E 与三类发布构建须在推送前另行执行。
...
- 六类 active 业务台账的共享控制条新增四个会话级快捷筛选芯片：“日期已填 / 待补日期”和“有附件 / 无附件”。同一维度互斥，重复点击会恢复“全部”；日期与附件两个维度可以同时启用，并继续与关键词、原结构化筛选和排序按 AND 组合。
- 日期口径与事务日历保持一致：任务使用 `deadline`、会议使用 `meetingTime`、文件使用 `docDate`、外出使用 `researchTime`、用章使用 `sealTime`、物资使用 `handlerTime`。日期必须通过有效四位年份校验；空值、非法日期和六位年份均进入“待补日期”。会议原“已安排 / 待补充”下拉也改用相同的严格日期时间校验，避免两个筛选控件口径冲突。
- “有附件 / 无附件”只判断当前记录顶层本机附件引用，不读取附件正文、哈希、Base64 或任务阶段中的历史附件。筛选只消费当前已加载 active 数组，不修改源顺序，不进入 IndexedDB、local/sessionStorage、快照、私有同步、API、IPC 或网络；物资账面库存继续由全部 active 物资流水计算。
- 四个芯片使用原生按钮、Lucide 图标、`aria-pressed` 和模块化分组名称，支持 Tab、Space 与 Enter。390×844 下按两列换行且每项为 44px；1440×900 与移动实测均无横向溢出。完整主 E2E 首轮发现共享详情按钮因子像素取整仅为 `43.999877px`，现固定为 45px 后聚焦回归及完整套件通过。
- 失败优先阶段领域测试为 3 项失败、UI 合约为 1 项失败；实现后领域聚焦 9/9、全部 workspace 193 项测试、UI 合约 23/23、主 E2E `101 passed / 11 skipped`、Internet 7/7、Intranet 3/3，以及公开/互联网/内网三类 Web 构建全部通过。独立 Chromium 149 证据确认桌面组合计数 `1 / 2`、移动四芯片均 44px、清除后全部复位、零动作请求/错误/横向溢出。Pages、桌面矩阵、12 个安装包、SHA256SUMS 和线上 PWA 验收均已完成。

## v0.7.21

- 六类 active 业务详情新增“复制记录摘要”按钮。摘要首行显示模块和记录标题，随后按固定白名单输出可见状态/类型、业务日期、人员/单位、摘要、备注、当前 active 关系标题、附件数量和最近更新时间；多行值使用缩进续行，便于粘贴到邮件、聊天或文稿中核对。
- 摘要复用已审计的六类只读字段展示模型，并在纯函数层拒绝 trash/purged 记录；不读取或输出内部 ID、删除/墓碑字段、迁移原文、来源版本、附件 ID/正文/Base64/哈希、API Key、访问码、中转密码或 AI 原文。关联任务/文件只显示当前 active 标题，附件只显示数量。
- 复制只在用户显式点击后向当前设备系统剪贴板写入纯文本，不写 IndexedDB、快照、私有同步、访问轨迹、星标或业务记录，不发起网络请求，也不会自动发送或粘贴。代码等待 `navigator.clipboard.writeText` 成功后才显示成功提示；权限拒绝或 API 不可用显示明确失败提示。
- 新增四项摘要领域测试、UI 合约和双视口聚焦 E2E，覆盖六类字段顺序、长中文/多行续行、active 关系、敏感字段排除、源对象不变、剪贴板失败、IndexedDB 写入哨兵、零外联、移动 44px 控件和无横向溢出。完整 Pages/Desktop/Release 证据将在推送后补入本节。

## v0.7.20

- 六类 active 业务详情新增“对比记录”入口。当前记录固定为左侧基准，目标从同模块 active 记录中选择，默认优先当前筛选结果的下一条、上一条，再回退到第一条其他记录；回收站、永久删除墓碑、历史档案和迁移原件不会进入候选。
